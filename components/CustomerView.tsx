    ...
    const handleGetLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                if (form) {
                    (form.elements.namedItem('googleMapsLink') as HTMLInputElement).value = link;
                }
                alert("Ubicación capturada. Puedes añadir referencias si es necesario.");
            ...
            if (isTableSession) {
                ...
                if (isFinalClosing) {
                    ...
                    const msg = [
                        ...
                        `💰 Ronda Actual: $${cartTotal.toFixed(2)}`, (sessionItems.length > 0) ? `📈 *Total Acumulado Mesa: $${(sessionTotal + cartTotal).toFixed(2)}*` : '',
                    ].filter(Boolean).join('\n');
                    window.open(`https://wa.me/${settings.branch.whatsappNumber}?text=${encodeURIComponent(msg)}`, '_blank');
                    clearCart();
                    setView('confirmation');
                ...
                const newOrderData: Omit<Order, 'id' | 'createdAt'> = { customer, items: cartItems, total: cartTotal, status: OrderStatus.Pending, orderType, paymentStatus: 'pending', generalComments: tipAmount > 0 ? `Propina: $${tipAmount}` : undefined };
                await saveOrder(newOrderData);
                const itemsStr = cartItems.map(i => `• ${i.quantity}x ${i.name}`).join('\n');
                let msg: string;
                ...
                if (orderType === OrderType.Delivery) {
                     msg = [
                        `*NUEVO PEDIDO A DOMICILIO*`, `📍 *${settings.company.name}*`, `--------------------------------`,
                        `👤 Cliente: ${customer.name}`, `📱 Tel: ${customer.phone}`, `--------------------------------`,
                        `🏠 *Dirección:*`,
                        `· ${customer.address.calle} #${customer.address.numero}, ${customer.address.colonia}`,
                        customer.address.referencias && `· Ref: ${customer.address.referencias}`,
                        customer.address.googleMapsLink && `· Ubicación: ${customer.address.googleMapsLink}`,
                        `--------------------------------`, itemsStr, `--------------------------------`,
                        `💰 *Total: $${cartTotal.toFixed(2)}*`, `💳 Método: ${paymentMethod}`
                    ].filter(Boolean).join('\n');
                } else { // TakeAway
                    msg = [
                        `*NUEVO PEDIDO PARA RECOGER*`, `📍 *${settings.company.name}*`, `--------------------------------`,
                        `👤 Cliente: ${customer.name}`, `📱 Tel: ${customer.phone}`, `--------------------------------`,
                        itemsStr, `--------------------------------`,
                        `💰 *Total: $${cartTotal.toFixed(2)}*`, `💳 Método: ${paymentMethod}`
                    ].filter(Boolean).join('\n');
        ...
        <div className="bg-gray-950 min-h-screen text-gray-100 font-sans selection:bg-emerald-500/20 pb-safe">
            <div className="container mx-auto max-w-md bg-gray-900 min-h-screen relative shadow-2xl border-x border-gray-800 flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto pb-48">
                    ...
                    {view === 'checkout' && ( <form id="checkout-form" onSubmit={e => { e.preventDefault(); const fd = new FormData(e.currentTarget); const customer: Customer = { name: fd.get('name') as string || customerName, phone: fd.get('phone') as string, address: {calle: fd.get('calle') as string, numero: fd.get('numero') as string, colonia: fd.get('colonia') as string, referencias: fd.get('referencias') as string, googleMapsLink: fd.get('googleMapsLink') as string } }; handleOrderAction(customer, fd.get('payment') as PaymentMethod || 'Efectivo', parseFloat(fd.get('tip') as string) || 0, (e.currentTarget.querySelector('input[name="proof"]') as any)?.dataset.url); }} className="p-6 space-y-6 animate-fade-in"> {isTableSession ? ( <> {(!customerName) && <input name="name" type="text" className="w-full bg-gray-800 rounded p-3" placeholder="¿A nombre de quién?" required />} {isFinalClosing && (<div className="space-y-4 p-4 bg-gray-800/30 rounded-xl"> <h3 className="font-bold">Pago Final</h3> <select name="payment" className="w-full bg-gray-800 p-3 rounded"><option>Efectivo</option><option>Tarjeta</option></select> {settings.payment.showTipField && <input name="tip" type="number" className="w-full bg-gray-800 p-3 rounded" placeholder="Propina (opcional)"/>} <input name="proof" type="file" className="w-full text-sm"/> </div>)} </> ) : ( <> <div className="space-y-4 p-4 bg-gray-800/30 rounded-xl"><h3 className="font-bold">TUS DATOS</h3><input name="name" type="text" className="w-full bg-gray-800 rounded p-3" placeholder="Nombre" required /><input name="phone" type="tel" className="w-full bg-gray-800 rounded p-3" placeholder="WhatsApp" required /></div> {orderType === OrderType.Delivery && (<div className="space-y-4 p-4 bg-gray-800/30 rounded-xl"><h3 className="font-bold">DIRECCIÓN DE ENVÍO</h3><input type="hidden" name="googleMapsLink" /><input name="calle" className="w-full bg-gray-800 rounded p-3" placeholder="Calle / Avenida" required /><input name="numero" className="w-full bg-gray-800 rounded p-3" placeholder="Nro Casa/Apto" required /><input name="colonia" className="w-full bg-gray-800 rounded p-3" placeholder="Colonia / Sector" required /><textarea name="referencias" className="w-full bg-gray-800 rounded p-3" placeholder="Referencias (ej. casa amarilla)"></textarea><button type="button" onClick={handleGetLocation} disabled={isGettingLocation} className="w-full flex items-center justify-center gap-2 bg-indigo-600 py-3 rounded-lg font-bold disabled:opacity-50"><IconLocationMarker className="h-5 w-5"/> {isGettingLocation ? 'Obteniendo...' : 'Usar ubicación actual'}</button></div>)} <div className="space-y-4 p-4 bg-gray-800/30 rounded-xl"><h3 className="font-bold">MÉTODO DE PAGO</h3> {(orderType === OrderType.Delivery ? settings.payment.deliveryMethods : settings.payment.pickupMethods).map(m => (<label key={m} className="flex items-center gap-3 p-3 bg-gray-800 rounded-lg"><input type="radio" name="payment" value={m} defaultChecked={m === 'Efectivo'} className="accent-emerald-500 h-5 w-5"/><span>{m}</span></label>))}</div> </> )} <div className="pt-4"><button type="submit" className="w-full bg-emerald-600 py-4 rounded-lg font-bold uppercase">{isFinalClosing ? 'Cerrar Mesa y Pagar' : (isTableSession ? 'Enviar a Cocina' : 'Enviar Pedido')}</button></div> </form> )}
                </div>
