    ...
    return (
        <div className="mb-6 p-4 bg-indigo-900/30 border border-indigo-500/30 rounded-2xl flex items-start gap-3 animate-fade-in shadow-lg shadow-indigo-500/10">
            ...
            <div className="flex-1">
                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">IA SUGIERE</p>
                {loading ? (
...
export default function CustomerView() {
    ...
    useEffect(() => {
        ...
        // Si entra por QR y no hay sesión activa, iniciamos una nueva
    ...
    // Lógica de localización para modo Online
    const handleGetLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                // Buscamos el input por nombre en el formulario actual
    ...
    const handleOrderAction = async (customer: Customer, paymentMethod: PaymentMethod, tipAmount: number, paymentProof?: string | null) => {
        ...
        try {
            if (isFinalClosing && isTableSession) {
                // --- FLUJO: CIERRE DE CUENTA FINAL (MESA) ---
                const msg = [
                ...
                // --- FLUJO: ENVIAR RONDA (O PEDIDO ONLINE NORMAL) ---
                ...
                if (isTableSession) {
                    ...
                    // FLUJO ORIGINAL RESTAURADO PARA DELIVERY/RECOGER
                    if (orderType === OrderType.Delivery) {
                        msg = [
                            `🧾 *PEDIDO A DOMICILIO*`, `📍 *${settings.company.name.toUpperCase()}*`, `--------------------------------`,
                            `👤 Cliente: ${customer.name}`, `📱 Tel: ${customer.phone}`,
                            `🏠 Dir: ${customer.address.calle} ${customer.address.numero}, ${customer.address.colonia}`,
                        ...
                        msg = [
                            `🥡 *PEDIDO PARA RECOGER*`, `📍 *${settings.company.name.toUpperCase()}*`, `--------------------------------`,
                            `👤 Cliente: ${customer.name}`, `📱 Tel: ${customer.phone}`,
                            `--------------------------------`, itemsStr, `--------------------------------`,
                            `💰 Total: $${cartTotal.toFixed(2)}`, `💳 Método: ${paymentMethod}`
                        ].filter(Boolean).join('\n');
        ...
        <div className="bg-[#0f172a] min-h-screen text-gray-100 font-sans selection:bg-emerald-500/20 pb-safe">
            <div className="container mx-auto max-w-md bg-[#0f172a] min-h-screen relative shadow-2xl border-x border-gray-800 flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto pb-48">
                    {view === 'menu' && (
                        <div className="animate-fade-in">
                            ...
                            <div className="relative pt-12 pb-8 flex flex-col items-center text-center">
                                ...
                                {/* Toggle oculto si es sesión de mesa activa para aislamiento total */}
                                {!isTableSession ? (
                                    ...
                                    return (
                                        <div key={cat.id} id={`cat-${cat.id}`}>
                                            <h3 className="text-lg font-black text-white uppercase tracking-tighter mb-4 flex items-center gap-2">
                                                {cat.name}
                                                <span className="bg-gray-800 text-gray-500 text-[10px] px-2 py-0.5 rounded-full font-bold">{products.length}</span>
                                            </h3>
                    ...
                    {view === 'checkout' && ( 
                        <form id="checkout-form" onSubmit={e => { e.preventDefault(); const fd = new FormData(e.currentTarget); const name = fd.get('name') as string || customerName; const payment = (fd.get('payment') as PaymentMethod) || 'Efectivo'; const proof = (e.currentTarget.elements.namedItem('proof') as any)?.dataset.url; handleOrderAction({ name, phone: fd.get('phone') as string || '', address: { colonia: fd.get('colonia') as string || '', calle: fd.get('calle') as string || '', numero: fd.get('numero') as string || '', googleMapsLink: fd.get('googleMapsLink') as string || '' } } as any, payment, 0, proof); }} className="p-6 space-y-6 animate-fade-in"> 
                            ...
                            {!isTableSession && orderType === OrderType.Delivery && (
                                <div className="space-y-4 p-6 bg-gray-800/30 border border-gray-800 rounded-[2rem]">
                                    ...
                                    <input name="referencias" className="w-full bg-gray-800 border-gray-700 rounded-xl p-4 outline-none focus:ring-2 focus:ring-emerald-500/40 text-sm font-bold text-white" placeholder="Referencias (Casa color azul, rejas negras...)" />
                            ...
                            <div className="pt-4">
                                <div className="flex justify-between font-black text-2xl mb-6 px-2">
                                    <span className="text-gray-500 text-[10px] font-black uppercase tracking-widest self-center">{isFinalClosing ? 'TOTAL A PAGAR' : 'TOTAL RONDA'}</span>
                                ...
                                <button type="submit" className="w-full bg-emerald-600 py-5 rounded-2xl font-black text-white flex items-center justify-center gap-4 active:scale-95 transition-all text-xs uppercase tracking-[0.2em] shadow-2xl shadow-emerald-900/30">
                                    ...
                                    {isFinalClosing ? 'SOLICITAR CIERRE DE CUENTA' : (isTableSession ? 'ENVIAR RONDA A COCINA' : 'REALIZAR PEDIDO')}
                                </button>
                            </div>
                        </form> 
                    )}
