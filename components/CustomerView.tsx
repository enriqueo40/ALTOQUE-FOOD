...
import { Product, Category, CartItem, Order, OrderStatus, Customer, AppSettings, PaymentMethod, OrderType, Personalization, Promotion, PersonalizationOption, Schedule, DaySchedule } from '../types';
...
export default function CustomerView() {
    ...
    const storeStatus = useMemo(() => settings ? getStoreStatus(settings.schedules) : { isOpen: false, message: 'Cargando...' }, [settings]);
                    ...
                    {view === 'menu' && (
                        <div className="animate-fade-in">
                            <div className="relative">
                                <div className="h-48 w-full bg-gradient-to-br from-purple-900 via-pink-700 to-amber-600">
                                    {settings.branch.coverImageUrl && (
                                        <img src={settings.branch.coverImageUrl} className="w-full h-full object-cover opacity-90" alt="Restaurant Cover" />
                                    )}
                                    <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/60 to-transparent"></div>
                                </div>
                            </div>
                            
                            <div className="pt-8 pb-6 flex flex-col items-center text-center -mt-16 relative z-10">
                                <div className="w-24 h-24 bg-gray-800 rounded-full p-1 shadow-2xl mb-4 border-4 border-gray-900 overflow-hidden">
                                    {settings.branch.logoUrl ? 
                                        <img src={settings.branch.logoUrl} className="w-full h-full object-cover rounded-full" alt="Restaurant Logo"/> 
                                        : <div className="w-full h-full flex items-center justify-center font-black text-emerald-400 text-3xl bg-gray-700">{settings.company.name.slice(0,2)}</div>
                                    }
                                </div>

                                <h1 className="text-2xl font-bold text-white mt-0 tracking-tight">{settings.company.name}</h1>
                                <p className="text-sm text-gray-400 mt-1">{settings.branch.alias}</p>

                                <div className="mt-4 space-y-2 text-xs">
                                    <div className="flex items-center justify-center gap-2">
                                        <span className={`w-2 h-2 rounded-full ${storeStatus.isOpen ? 'bg-green-400 animate-pulse' : 'bg-gray-500'}`}></span>
                                        <span className={`font-bold ${storeStatus.isOpen ? 'text-green-400' : 'text-gray-500'}`}>{storeStatus.message}</span>
                                        <button onClick={() => setIsScheduleModalOpen(true)} className="text-gray-400 underline hover:text-white transition-colors">Ver horarios</button>
                                    </div>
                                    
                                    {settings.branch.googleMapsLink && settings.branch.fullAddress && (
                                        <a href={settings.branch.googleMapsLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-1.5 text-gray-500 hover:text-gray-300 transition-colors pt-1">
                                ...
                                <div className="mt-6">
                                    {!isTableSession ? (
                                        <div className="w-full max-w-xs bg-gray-800 rounded-full p-1 flex relative border border-gray-700">
                                            ...
                                            <button onClick={() => setOrderType(OrderType.Delivery)} className={`flex-1 relative z-10 py-2.5 text-xs font-black uppercase tracking-wider transition-colors ${orderType === OrderType.Delivery ? 'text-white' : 'text-gray-400'}`}>DOMICILIO</button>
                                            <button onClick={() => setOrderType(OrderType.TakeAway)} className={`flex-1 relative z-10 py-2.5 text-xs font-black uppercase tracking-wider transition-colors ${orderType === OrderType.TakeAway ? 'text-white' : 'text-gray-400'}`}>RECOGER</button>
                                        </div>
                                    ) : (
                                        ...
                                        <div key={cat.id}>
                                            ...
                                            <div className="space-y-3">
                                                {products.map(p => {
                                                    ...
                                                    return (
                                                        <div key={p.id} onClick={() => setSelectedProduct(p)} className="bg-gray-800 p-3 rounded-2xl flex gap-4 cursor-pointer transition-transform active:scale-95 border border-transparent hover:border-emerald-500/30">
                                                            <div className="relative shrink-0">
                                                                ...
                                                                <button 
                                                                    ...
                                                                    className="absolute -bottom-2 -right-2 bg-emerald-500 p-2 rounded-full shadow-lg border-2 border-gray-900 active:scale-90 transition-transform">
