...
const PairingAI: React.FC<{ items: CartItem[], allProducts: Product[] }> = ({ items, allProducts }) => {
    ...
    useEffect(() => {
        let isMounted = true;
        // The service will read from localStorage, so we only need to check current cart items
        if (items.length > 0) {
            setLoading(true);
...
export default function CustomerView() {
    ...
    useEffect(() => {
        if (tableInfo) {
            ...
            setOrderType(OrderType.DineIn); // Lock order type
        } else {
            // This case handles explicit session clearing.
    ...
    useEffect(() => {
        ...
        // Only set table info if it doesn't already exist in state (from localStorage)
    ...
    const handleOrderAction = async (customer: Customer, paymentMethod: PaymentMethod, tipAmount: number, paymentProof?: string | null) => {
        ...
        try {
            if (isFinalClosing && isTableSession) {
                ...
                // --- NUEVA RONDA (PEDIDO A COCINA) O PEDIDO NORMAL ---
                ...
                if (isTableSession) {
                    ...
                    // This is the existing logic for delivery/take-away, untouched.
                    msg = [
    ...
    return (
        <div className="bg-gray-950 min-h-screen text-gray-100 font-sans selection:bg-emerald-500/20 pb-safe">
            <div className="container mx-auto max-w-md bg-gray-900 min-h-screen relative shadow-2xl border-x border-gray-800 flex flex-col">
                ...
                {view !== 'menu' && (
                    <Header 
                        title={view === 'cart' ? 'MI RONDA' : view === 'account' ? 'MI CUENTA' : (isFinalClosing ? 'CERRAR MESA' : 'CONFIRMAR PEDIDO')} 
                        onBack={() => {
                            if (view === 'checkout') {
                            ...
                            <div className="space-y-4">
                                ...
                                <p className="text-gray-500 text-sm">
                                    {isFinalClosing 
                                        ...
                                        : 'Tu ronda ha sido enviada. Puedes seguir pidiendo más cosas.'}
                                </p>
                            </div>
                            <button onClick={isFinalClosing ? handleStartNewSession : () => setView('menu')} className="w-full max-w-xs bg-gray-800 text-white py-4 rounded-xl font-bold uppercase tracking-widest text-xs">
                                {isFinalClosing ? 'INICIAR NUEVO PEDIDO' : 'SEGUIR PIDIENDO'}
                            </button>
                        </div>
                    )}
                    
                    {view === 'cart' && ( <div className="p-5 animate-fade-in"> <PairingAI items={cartItems} allProducts={allProducts} /> <h2 className="text-xl font-black text-white mb-6 uppercase">Tu Ronda Actual</h2> <div className="space-y-4"> {cartItems.map(i => ( <div key={i.cartItemId} className="flex gap-4"> <img src={i.imageUrl} className="w-20 h-20 rounded-xl object-cover" /> <div className="flex-1"> <div className="flex justify-between"> <span className="font-bold">{i.name}</span> <span>${(i.price * i.quantity).toFixed(2)}</span> </div> <div className="flex items-center justify-between mt-2"> <div className="flex items-center bg-gray-800 rounded-lg"> <button onClick={() => updateQuantity(i.cartItemId, i.quantity - 1)} className="p-2"><IconMinus className="h-4 w-4"/></button> <span className="w-8 text-center text-sm font-bold">{i.quantity}</span> <button onClick={() => updateQuantity(i.cartItemId, i.quantity + 1)} className="p-2"><IconPlus className="h-4 w-4"/></button> </div> <button onClick={() => removeFromCart(i.cartItemId)} className="text-rose-500"><IconTrash/></button> </div> </div> </div> ))} </div> <div className="mt-8 pt-6 border-t border-gray-800"> <div className="flex justify-between font-black text-xl mb-6"> <span>TOTAL RONDA</span> <span>${cartTotal.toFixed(2)}</span> </div> <button disabled={cartItems.length === 0} onClick={() => { setIsFinalClosing(false); setView('checkout'); }} className="w-full bg-emerald-600 py-4 rounded-lg font-bold disabled:opacity-30 uppercase">{isTableSession ? 'Enviar a Cocina' : 'Ir a Pagar'}</button> </div> </div> )}
                    {view === 'account' && ( <div className="p-6 animate-fade-in"> <div className="bg-gray-800/30 p-6 rounded-2xl border border-gray-800"> <h3 className="text-sm font-bold text-emerald-400 uppercase mb-4">Cuenta Acumulada</h3> <div className="space-y-3"> {sessionItems.map((item, idx) => ( <div key={idx} className="flex justify-between text-sm border-b border-gray-800 pb-3"> <span className="font-medium text-gray-300">{item.quantity}x {item.name}</span> <span className="font-bold">${(item.price * item.quantity).toFixed(2)}</span> </div> ))} {sessionItems.length === 0 && <p className="text-center text-gray-500 py-4 text-sm">Aún no has pedido nada en esta sesión.</p>} </div> <div className="mt-6 pt-6 border-t border-gray-800 flex justify-between items-center"> <span className="text-gray-400 text-sm font-bold">TOTAL A PAGAR</span> <span className="text-2xl font-black text-white">${sessionTotal.toFixed(2)}</span> </div> </div> <button onClick={() => { setIsFinalClosing(true); setView('checkout'); }} className="w-full mt-6 bg-white text-gray-900 py-4 rounded-lg font-bold uppercase">Pedir la Cuenta / Pagar</button> </div> )}
                    {view === 'checkout' && ( <form onSubmit={e => { e.preventDefault(); const fd = new FormData(e.currentTarget); handleOrderAction({ name: fd.get('name') as string || customerName, phone: fd.get('phone') as string, address: {calle:'', numero:'', colonia:''} } as Customer, fd.get('payment') as PaymentMethod || 'Efectivo', parseFloat(fd.get('tip') as string) || 0, (e.currentTarget.querySelector('input[name="proof"]') as any)?.dataset.url); }} className="p-6 space-y-6 animate-fade-in"> {(!customerName && isTableSession) && <input name="name" type="text" className="w-full bg-gray-800 rounded p-3" placeholder="¿A nombre de quién?" required />} {!isTableSession && (<> <input name="name" type="text" className="w-full bg-gray-800 rounded p-3" placeholder="Nombre" required /> <input name="phone" type="tel" className="w-full bg-gray-800 rounded p-3" placeholder="WhatsApp" required /> {orderType === OrderType.Delivery && (<> <input name="calle" className="w-full bg-gray-800 rounded p-3" placeholder="Calle" required /> <input name="numero" className="w-full bg-gray-800 rounded p-3" placeholder="Número" required /> <input name="colonia" className="w-full bg-gray-800 rounded p-3" placeholder="Colonia" required /> </>)} </> )} {isFinalClosing && isTableSession && (<div className="space-y-4 p-4 bg-gray-800/30 rounded-xl"> <h3 className="font-bold">Pago Final</h3> <select name="payment" className="w-full bg-gray-800 p-3 rounded"><option>Efectivo</option><option>Tarjeta</option></select> {settings.payment.showTipField && <input name="tip" type="number" className="w-full bg-gray-800 p-3 rounded" placeholder="Propina (opcional)"/>} <input name="proof" type="file" className="w-full text-sm"/> </div>)} <div className="pt-4"> <button type="submit" className="w-full bg-emerald-600 py-4 rounded-lg font-bold uppercase">{isFinalClosing ? 'Cerrar Mesa y Pagar' : (isTableSession ? 'Enviar a Cocina' : 'Enviar Pedido')}</button> </div> </form> )}
                </div>

                {selectedProduct && ( <div className="fixed inset-0 z-50 flex items-end"><div className="absolute inset-0 bg-black/80" onClick={() => setSelectedProduct(null)}></div><div className="bg-gray-900 w-full max-w-md rounded-t-3xl p-8 relative z-10 animate-slide-up"><h2 className="text-2xl font-bold">{selectedProduct.name}</h2><p className="text-gray-400 my-4">{selectedProduct.description}</p><button onClick={() => { addToCart(selectedProduct, 1); setSelectedProduct(null); }} className="w-full bg-emerald-600 py-4 rounded-lg font-bold">Añadir por ${getDiscountedPrice(selectedProduct, allPromotions).price.toFixed(2)}</button></div></div>)}
                ...
                {view === 'menu' && (
                    <div className="fixed bottom-6 left-4 right-4 max-w-md mx-auto z-40 flex flex-col gap-3">
                        {isTableSession && sessionItems.length > 0 && (
                            <button onClick={() => setView('account')} className="w-full bg-gray-800/80 backdrop-blur-md text-white font-bold py-3 px-5 rounded-xl flex justify-between items-center border border-emerald-500/20 shadow-lg">
                                <span>Mi Cuenta Acumulada</span>
                                <span>${sessionTotal.toFixed(2)}</span>
                            </button>
                        )}
                        {itemCount > 0 && (
                            <button onClick={() => setView('cart')} className="w-full bg-emerald-600 text-white font-black py-4 px-6 rounded-2xl flex justify-between items-center shadow-2xl">
                                <div className="flex items-center gap-3">
                                    <div className="bg-emerald-800 w-7 h-7 flex items-center justify-center rounded-lg">{itemCount}</div>
                                    <span>{isTableSession ? 'Ver Ronda Actual' : 'Ver Pedido'}</span>
                                </div>
