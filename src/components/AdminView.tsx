...
import { IconComponent, IconHome, IconMenu, IconAvailability, IconShare, IconTutorials, IconOrders, IconAnalytics, IconChatAdmin, IconLogout, IconSearch, IconBell, IconEdit, IconPlus, IconTrash, IconSparkles, IconSend, IconMoreVertical, IconExternalLink, IconCalendar, IconChevronDown, IconX, IconReceipt, IconSettings, IconStore, IconDelivery, IconPayment, IconClock, IconTableLayout, IconPrinter, IconChevronUp, IconPencil, IconDuplicate, IconGripVertical, IconPercent, IconInfo, IconTag, IconLogoutAlt, IconSun, IconMoon, IconArrowLeft, IconWhatsapp, IconQR, IconLocationMarker, IconUpload, IconCheck, IconBluetooth, IconUSB, IconToggleOn, IconToggleOff, IconMap } from '../constants';
...
const Dashboard: React.FC = () => {
    ...
    const previousDaySales = totalSales * 0.9; // Simulation
    const previousDayOrders = Math.floor(totalOrders * 0.9); // Simulation
                                        ...
                                        <div className="pt-2 border-t dark:border-gray-600 mt-2">
                                            {order.customer.address.googleMapsLink ? (
                                                <a 
                                                    ...
                                                    className="mb-2 flex items-center gap-2 text-blue-500 hover:underline font-semibold"
                                                >
                                                    <IconMap className="h-4 w-4" /> Ver ubicación GPS
                                                </a>
                                            ) : null}
...
const OrderCard: React.FC<{ order: Order; onClick: () => void }> = ({ order, onClick }) => {
    // Helper to get styles based on order type
    const getTypeConfig = (type: OrderType) => {
        switch (type) {
            ...
            case OrderType.DineIn:
                return {
                    bg: 'bg-orange-100 text-orange-700 dark:bg-orange-900/50 dark:text-orange-300',
                    icon: <IconTableLayout className="h-4 w-4"/>,
                    label: 'Mesa'
                };
            default: // TakeAway
                return {
    ...
    const typeConfig = getTypeConfig(order.orderType || OrderType.TakeAway);
    ...
    return (
        ...
        <div className="flex justify-between items-start mb-3">
            <div className="flex items-center gap-2">
                 <span className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold ${typeConfig.bg}`}>
                    {typeConfig.icon}
                 </span>
                 <div>
                     ...
                     <div className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
                        ...
                        {order.tableId && (
                            <span className="bg-orange-50 text-orange-700 px-1.5 rounded border border-orange-100 font-medium ml-1">
                                {order.tableId}
                            </span>
                        )}
                     </div>
...
const OrderListView: React.FC<{ orders: Order[], onOrderClick: (order: Order) => void }> = ({ orders, onOrderClick }) => {
    // Helper to get badge styles for list view
    const getTypeBadgeInfo = (type: OrderType) => {
        switch (type) {
            case OrderType.Delivery:
                return { class: 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300', label: 'Domicilio', icon: <IconDelivery className="h-3 w-3 mr-1"/> };
            case OrderType.DineIn:
                return { class: 'bg-orange-50 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300', label: 'En Mesa', icon: <IconTableLayout className="h-3 w-3 mr-1"/> };
            default:
                return { class: 'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300', label: 'Para llevar', icon: <IconStore className="h-3 w-3 mr-1"/> };
        }
    };
        ...
        <div className="bg-white dark:bg-gray-800 shadow-sm rounded-lg border dark:border-gray-700 overflow-hidden">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {orders.map(order => {
                        const badgeInfo = getTypeBadgeInfo(order.orderType || OrderType.TakeAway);
                        return (
                            <tr key={order.id} onClick={() => onOrderClick(order)} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer transition-colors group">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="flex flex-col items-start gap-1">
                                        <span className={`px-2 py-1 inline-flex items-center text-xs leading-5 font-semibold rounded-md ${badgeInfo.class}`}>
                                            {badgeInfo.icon} {badgeInfo.label}
                                        </span>
                                        {order.tableId && (
                                            <span className="text-[10px] text-gray-500 font-mono bg-gray-100 dark:bg-gray-700 px-1 rounded">
                                                {order.tableId}
                                            </span>
                                        )}
                                    </div>
