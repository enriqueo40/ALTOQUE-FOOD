...
const ShippingSettingsView: React.FC<{ onSave: () => Promise<void>; settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }> = ({ onSave, settings, setSettings }) => {
    ...
    const inputClasses = "w-24 text-center px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm text-gray-900 dark:text-white";
                ...
                <div className="space-y-4">
                    <div>
                        <select 
                            ...
                            className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm text-gray-900 dark:text-white"
                        >
                           {Object.values(ShippingCostType).map(type => <option key={type} value={type}>{type}</option>)}
                        </select>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            {settings.shipping.costType === ShippingCostType.ToBeQuoted ? 'El precio de envío será acordado después de realizar el pedido.' : 
                             settings.shipping.costType === ShippingCostType.Free ? 'El envío no tendrá costo para el cliente.' : 
                             'El precio de envío no será calculado automáticamente.'}
                        </p>
                    </div>

                    {settings.shipping.costType === ShippingCostType.Fixed && (
                        <div className="bg-gray-50 dark:bg-gray-700/30 p-4 rounded-lg border border-gray-200 dark:border-gray-700 animate-fade-in">
                            ...
                            <div className="relative rounded-md shadow-sm w-full">
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setSettings(p => ({...p, shipping: {...p.shipping, fixedCost: isNaN(val) ? 0 : val}}));
                                    }}
                                    className="block w-full rounded-md border-gray-300 pl-7 pr-12 focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm py-2 bg-white dark:bg-gray-800 dark:border-gray-600 dark:text-white placeholder-gray-400"
                                    placeholder="0.00"
                                />
             ...
             <SettingsCard title="Tiempo para pedidos a domicilio" onSave={onSave} onCancel={handleCancel}>
                ...
                <div className="flex items-center gap-4 mt-2">
                    <div className="flex items-center gap-2">
                         ...
                         <span className="text-gray-700 dark:text-gray-300">mins</span>
                    </div>
                 ...
                 <h4 className="font-bold mt-6 text-gray-900 dark:text-gray-100">Tiempo para pedidos para recoger</h4>
                 <p className="text-sm text-gray-500 dark:text-gray-400">Desde que el cliente hace su pedido.</p>
