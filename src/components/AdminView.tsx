                                        ...
                                        <div className="pt-2 border-t dark:border-gray-600 mt-2">
                                            ...
                                            {order.customer.address.googleMapsLink && (
                                                <a 
                                                    ...
                                                    className="mt-4 flex items-center justify-center gap-2 w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors font-bold shadow-sm"
                                                >
                                                    <IconLocationMarker className="h-5 w-5"/> Ver ubicación GPS
                                                </a>
                                            )}
                                        </div>
                                    )}
                    ...
                    <div className="h-[calc(100vh-220px)] flex flex-col">
                         <div className="flex justify-between items-center mb-4 px-1">
                            <div className="flex items-center space-x-2 bg-white dark:bg-gray-800 p-1 rounded-lg border dark:border-gray-700 shadow-sm">
                                <button onClick={() => setViewMode('board')} className={`p-2 rounded-md ${viewMode === 'board' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50' : 'text-gray-500'}`}><IconTableLayout className="h-5 w-5"/></button>
                                <button onClick={() => setViewMode('list')} className={`p-2 rounded-md ${viewMode === 'list' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50' : 'text-gray-500'}`}><IconMenu className="h-5 w-5"/></button>
                            </div>
                            <div className="flex items-center gap-3">
                                <button onClick={() => setStoreOpen(!storeOpen)} className={`px-4 py-2 rounded-lg border text-sm font-semibold transition-colors ${storeOpen ? 'bg-green-900/20 text-green-400' : 'bg-red-900/20 text-red-400'}`}>
                                    {storeOpen ? 'Abierta' : 'Cerrada'}
                                </button>
                                <button onClick={() => setIsNewOrderModalOpen(true)} className="bg-white text-gray-900 px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shadow">+ Manual</button>
                            </div>
                         </div>
            ...
            case 'panel-mesas':
                return (
                    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900 p-4">
                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border mb-6 grid grid-cols-4 divide-x dark:divide-gray-700">
                            <div className="p-4 text-center"><p className="text-2xl font-bold">{tableStats.requestingBill}</p><p className="text-[10px] uppercase text-gray-500">Cuentas</p></div>
                            <div className="p-4 text-center"><p className="text-2xl font-bold">{tableStats.requestingWaiter}</p><p className="text-[10px] uppercase text-gray-500">Mesero</p></div>
                            <div className="p-4 text-center"><p className="text-2xl font-bold">{tableStats.pendingOrders}</p><p className="text-[10px] uppercase text-gray-500">Comandas</p></div>
                            <div className="p-4 text-center"><p className="text-2xl font-bold">{tableStats.activeTables}</p><p className="text-[10px] uppercase text-gray-500">Activas</p></div>
                        </div>
                        <div className="mb-6 flex gap-2 overflow-x-auto">
                            {zones.map(zone => (
                                <button key={zone.id} onClick={() => setActiveZoneId(zone.id)} className={`px-6 py-2 rounded-lg border text-sm font-medium whitespace-nowrap ${activeZoneId === zone.id ? 'bg-green-50 text-green-600 border-green-500' : 'bg-white dark:bg-gray-800'}`}>{zone.name}</button>
                            ))}
                        </div>
                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border flex-1 p-8 overflow-auto min-h-[400px]">
                            {activeZone ? (
                                <div className="grid gap-6" style={{ gridTemplateColumns: `repeat(${activeZone.cols}, minmax(80px, 1fr))`, gridTemplateRows: `repeat(${activeZone.rows}, minmax(80px, 1fr))` }}>
                                    {activeZone.tables.map(table => {
                                        ...
                                        return (
                                            <div key={table.id} onClick={() => isOccupied && order && setSelectedOrder(order)} style={{ gridRow: `${table.row} / span ${table.height}`, gridColumn: `${table.col} / span ${table.width}` }} className={`relative flex flex-col items-center justify-center cursor-pointer border-2 ${table.shape === 'round' ? 'rounded-full aspect-square' : 'rounded-xl'} ${isOccupied ? 'bg-red-50 border-red-400' : 'bg-gray-50'}`}>
                                                <span className={`text-2xl font-bold ${isOccupied ? 'text-red-500' : 'text-gray-400'}`}>{table.name}</span>
                                                {isOccupied && <div className="mt-1 px-2 py-0.5 bg-white rounded text-[10px] font-medium shadow-sm">Ocupada</div>}
                                            </div>
                                        );
                                    })}
                                </div>
                            ) : <div className="flex items-center justify-center h-full text-gray-400">Selecciona una zona</div>}
                        </div>
                    </div>
                );
            default: return null;
        ...
        <div className="h-full flex flex-col">
            <div className="border-b dark:border-gray-700">
                <nav className="-mb-px flex space-x-8">
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`${activeTab === tab.id ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-gray-500'} py-4 px-1 border-b-2 font-medium text-sm`}>{tab.title}</button>
                    ))}
                </nav>
            </div>
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            ...
            <div className="flex space-x-2">
                <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Ej: ¿Cuáles son los productos más populares?" className="flex-1 px-4 py-2 border dark:border-gray-600 rounded-lg bg-transparent" />
                <button onClick={handleGetInsights} disabled={isLoading} className="bg-indigo-600 text-white px-6 py-2 rounded-lg flex items-center space-x-2 hover:bg-indigo-700 disabled:bg-indigo-300">
                    <IconSparkles />
                    <span>{isLoading ? 'Analizando...' : 'Insights'}</span>
                </button>
            </div>
            {insights && <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border prose dark:prose-invert max-w-none"><pre className="whitespace-pre-wrap font-sans bg-transparent p-0">{insights}</pre></div>}
        </div>
    );
};
...
const Messages: React.FC = () => {
    ...
    const handleSendMessage = () => {
        ...
        const updated = { ...selectedConversation, messages: [...selectedConversation.messages, message], lastMessage: message.text, lastMessageTimestamp: message.timestamp };
        ...
        <div className="flex h-[calc(100vh-160px)] bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            <div className="w-1/3 border-r dark:border-gray-700 overflow-y-auto">
                {conversations.map(conv => (
                    <div key={conv.id} onClick={() => setSelectedConversation(conv)} className={`p-4 cursor-pointer border-l-4 ${selectedConversation?.id === conv.id ? 'border-indigo-500 bg-gray-50' : 'border-transparent'}`}><p className="font-semibold">{conv.customerName}</p><p className="text-sm truncate">{conv.lastMessage}</p></div>
                ))}
            </div>
                    ...
                    <>
                        <div className="flex-1 p-6 overflow-y-auto space-y-4">
                            {selectedConversation.messages.map(msg => (
                                <div key={msg.id} className={`flex ${msg.sender === 'admin' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-md p-3 rounded-lg ${msg.sender === 'admin' ? 'bg-indigo-500 text-white' : 'bg-gray-200'}`}><p>{msg.text}</p></div></div>
                            ))}
                        </div>
                        <div className="p-4 border-t flex items-center space-x-2">
                            <input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSendMessage()} placeholder="Escribe..." className="flex-1 px-4 py-2 border rounded-full bg-transparent" />
                            <button onClick={handleSendMessage} className="bg-indigo-600 text-white rounded-full p-3"><IconSend /></button>
                        </div>
                    </>
                ) : <div className="flex items-center justify-center h-full">Selecciona un chat</div>}
            </div>
        </div>
    );
};
...
const AvailabilityView: React.FC = () => {
    ...
    useEffect(() => {
        Promise.all([getProducts(), getCategories()]).then(([p, c]) => { setProducts(p); setCategories(c); setIsLoading(false); });
    }, []);
    ...
    const handleToggle = async (productId: string, current: boolean) => {
        ...
        await updateProductAvailability(productId, !current);
    };
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow space-y-6">
            <input type="text" placeholder="Buscar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full px-4 py-2 border rounded-lg bg-transparent" />
            <div className="divide-y dark:divide-gray-700">
                {products.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(p => (
                    <div key={p.id} className="py-4 flex justify-between items-center">
                        <span>{p.name}</span>
                        <button onClick={() => handleToggle(p.id, p.available)} className={`px-4 py-1 rounded-full text-xs font-bold ${p.available ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
...
const SettingsCard: React.FC<{ title: string; children: React.ReactNode; onSave?: () => void; onCancel?: () => void; noActions?: boolean }> = ({ title, children, onSave, onCancel, noActions }) => (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border p-6">
        <h3 className="text-lg font-bold mb-6">{title}</h3>
        <div className="space-y-4">{children}</div>
        {!noActions && (
            <div className="mt-6 flex justify-end gap-x-3">
                <button onClick={onCancel} className="px-4 py-2 border rounded-md text-sm">Cancelar</button>
...
const SearchableDropdown: React.FC<{ options: Currency[], selected: Currency, onSelect: (option: Currency) => void }> = ({ options, selected, onSelect }) => {
    ...
    return (
        <div className="relative">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full text-left p-2 border rounded bg-transparent">{selected.name}</button>
            {isOpen && (
                <div className="absolute z-10 w-full bg-white dark:bg-gray-800 border rounded shadow-lg mt-1 max-h-60 overflow-y-auto">
                    <input type="text" value={search} onChange={e => setSearch(e.target.value)} className="w-full p-2 border-b bg-transparent" placeholder="Buscar..." />
                    {options.filter(o => o.name.toLowerCase().includes(search.toLowerCase())).map(o => (
                        <div key={o.code} onClick={() => { onSelect(o); setIsOpen(false); }} className="p-2 hover:bg-gray-100 cursor-pointer">{o.name}</div>
                    ))}
                </div>
            )}
        </div>
...
const GeneralSettings: React.FC<{ onSave: () => Promise<void>; settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }> = ({ onSave, settings, setSettings }) => (
    <SettingsCard title="General" onSave={onSave}>
        ...
        <input type="text" value={settings.company.name} onChange={e => setSettings(p => ({...p, company: {...p.company, name: e.target.value}}))} className="w-full p-2 border rounded bg-transparent"/>
        <label className="block text-sm font-medium mt-4">Divisa</label>
        <SearchableDropdown options={CURRENCIES} selected={settings.company.currency} onSelect={c => setSettings(p => ({...p, company: {...p.company, currency: c}}))} />
    </SettingsCard>
);
...
const BranchSettingsView: React.FC<{ onSave: () => Promise<void>; settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }> = ({ onSave, settings, setSettings }) => (
    <SettingsCard title="Sucursal" onSave={onSave}>
        <label className="block text-sm font-medium">Alias</label>
        <input type="text" value={settings.branch.alias} onChange={e => setSettings(p => ({...p, branch: {...p.branch, alias: e.target.value}}))} className="w-full p-2 border rounded bg-transparent"/>
        <label className="block text-sm font-medium mt-4">Dirección</label>
        <input type="text" value={settings.branch.fullAddress} onChange={e => setSettings(p => ({...p, branch: {...p.branch, fullAddress: e.target.value}}))} className="w-full p-2 border rounded bg-transparent"/>
    </SettingsCard>
);
...
const ShippingSettingsView: React.FC<{ onSave: () => Promise<void>; settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }> = ({ onSave, settings, setSettings }) => (
    <SettingsCard title="Envío" onSave={onSave}>
        <select value={settings.shipping.costType} onChange={e => setSettings(p => ({...p, shipping: {...p.shipping, costType: e.target.value as ShippingCostType}}))} className="w-full p-2 border rounded bg-transparent">
...
const PaymentSettingsView: React.FC<{ onSave: () => Promise<void>; settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }> = ({ onSave, settings, setSettings }) => (
    <SettingsCard title="Pago" onSave={onSave}>
        <label className="flex items-center"><input type="checkbox" checked={settings.payment.showTipField} onChange={e => setSettings(p => ({...p, payment: {...p.payment, showTipField: e.target.checked}}))} className="mr-2"/> Propinas</label>
    </SettingsCard>
);

const TimeInput: React.FC<{ value: string; onChange: (value: string) => void }> = ({ value, onChange }) => <input type="time" value={value} onChange={e => onChange(e.target.value)} className="p-2 border rounded bg-transparent"/>;
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-md w-full space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <TimeInput value={shift.start} onChange={v => setShift(s => ({...s, start: v}))} />
                    <TimeInput value={shift.end} onChange={v => setShift(s => ({...s, end: v}))} />
                </div>
                <div className="flex justify-end gap-2"><button onClick={onClose}>Cancelar</button><button onClick={() => { onSave(shift); onClose(); }} className="bg-emerald-600 text-white px-4 py-2 rounded">Guardar</button></div>
            </div>
        </div>
    );
};
...
const ScheduleModal: React.FC<{ isOpen: boolean, onClose: () => void, onSave: (name: string) => void }> = ({ isOpen, onClose, onSave }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-md w-full space-y-4">
                <h2 className="text-lg font-bold">Nuevo Horario</h2>
                <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full p-2 border rounded bg-transparent" />
                <div className="flex justify-end gap-2"><button onClick={onClose}>Cancelar</button><button onClick={() => onSave(name)} className="bg-emerald-600 text-white px-4 py-2 rounded">Crear</button></div>
            </div>
        </div>
    );
};
...
const HoursSettings: React.FC<{ onSave: () => Promise<void>; settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }> = ({ onSave, settings, setSettings }) => {
    const [activeId, setActiveId] = useState(settings.schedules[0]?.id || '');
    const [isShiftOpen, setIsShiftOpen] = useState(false);
    const [selDay, setSelDay] = useState<DaySchedule['day'] | null>(null);
    const [isSchedOpen, setIsSchedOpen] = useState(false);
    const active = settings.schedules.find(s => s.id === activeId);
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <select value={activeId} onChange={e => setActiveId(e.target.value)} className="p-2 border rounded bg-transparent">{settings.schedules.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
                <button onClick={() => setIsSchedOpen(true)} className="bg-emerald-600 text-white px-4 py-2 rounded">+ Horario</button>
            </div>
            <ul className="bg-white dark:bg-gray-800 border rounded divide-y dark:divide-gray-700">
                {active?.days.map(d => (
                    <li key={d.day} className="p-4 flex justify-between items-center">
                        <span className="w-24 font-semibold">{d.day}</span>
                        <div className="flex-1 flex flex-wrap gap-2">{d.shifts.map((s, i) => <div key={i} className="bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded text-xs">{s.start}-{s.end}</div>)}</div>
                        <button onClick={() => { setSelDay(d.day); setIsShiftOpen(true); }} className="text-emerald-600">+ Turno</button>
                    </li>
                ))}
            </ul>
            <ShiftModal isOpen={isShiftOpen} onClose={() => setIsShiftOpen(false)} onSave={s => setSettings(prev => ({...prev, schedules: prev.schedules.map(sch => sch.id === activeId ? {...sch, days: sch.days.map(day => day.day === selDay ? {...day, shifts: [...day.shifts, s]} : day)} : sch)}))} day={selDay || ''} />
            <ScheduleModal isOpen={isSchedOpen} onClose={() => setIsSchedOpen(false)} onSave={n => { const ns = { id: `sc-${Date.now()}`, name: n, days: settings.schedules[0].days.map(d => ({...d, shifts: []})) }; setSettings(p => ({...p, schedules: [...p.schedules, ns]})); setActiveId(ns.id); setIsSchedOpen(false); }} />
        </div>
    );
};
...
const ZonesAndTablesSettings: React.FC<{ zones: Zone[]; onAddZone: () => void; onEditZoneName: (zone: Zone) => void; onDeleteZone: (zoneId: string) => void; onEditZoneLayout: (zone: Zone) => void; }> = ({ zones, onAddZone, onEditZoneName, onDeleteZone, onEditZoneLayout }) => (
    <div className="space-y-6">
        <div className="flex justify-end"><button onClick={onAddZone} className="bg-emerald-600 text-white px-4 py-2 rounded">+ Zona</button></div>
        <div className="bg-white dark:bg-gray-800 border rounded divide-y dark:divide-gray-700">
            {zones.map(z => (
                <div key={z.id} className="p-4 flex justify-between items-center">
                    <input type="text" defaultValue={z.name} onBlur={e => onEditZoneName({...z, name: e.target.value})} className="bg-transparent font-bold"/>
                    <div className="flex gap-2">
                        <button onClick={() => onEditZoneLayout(z)} className="text-emerald-600">Layout</button>
                        <button onClick={() => onDeleteZone(z.id)} className="text-red-500">Borrar</button>
                    </div>
                </div>
            ))}
        </div>
...
const PrintingSettingsView: React.FC<{ onSave: () => Promise<void>; settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }> = ({ onSave, settings, setSettings }) => (
    <SettingsCard title="Impresión" onSave={onSave}>
        {Object.values(PrintingMethod).map(m => (
            <button key={m} onClick={() => setSettings(p => ({...p, printing: { method: m }}))} className={`w-full p-4 border rounded text-left ${settings.printing.method === m ? 'border-emerald-500 bg-emerald-50' : ''}`}>{m}</button>
        ))}
    </SettingsCard>
);
...
const ZoneEditor: React.FC<{ initialZone: Zone; onSave: (zone: Zone) => void; onExit: () => void; }> = ({ initialZone, onSave, onExit }) => {
    ...
    const isOccupied = (r: number, c: number, id: string | null = null) => zone.tables.some(t => t.id !== id && r >= t.row && r < t.row + t.height && c >= t.col && c < t.col + t.width);
    const add = (r: number, c: number) => { if(!isOccupied(r,c)) { const nt: Table = { id: crypto.randomUUID(), zoneId: zone.id, name: (zone.tables.length + 1).toString(), row: r, col: c, width: 1, height: 1, shape: 'square', status: 'available' }; setZone(z => ({...z, tables: [...z.tables, nt]})); setSelId(nt.id); } };
    const upd = (t: Table) => { setZone(z => ({...z, tables: z.tables.map(it => it.id === t.id ? t : it)})); };
    const sel = zone.tables.find(t => t.id === selId);
    return (
        <div className="fixed inset-0 bg-white dark:bg-gray-900 z-50 flex flex-col">
            <header className="p-4 border-b flex justify-between items-center"><button onClick={onExit} className="text-red-500">Salir</button><h2>{zone.name}</h2><button onClick={() => onSave(zone)} className="bg-emerald-600 text-white px-4 py-2 rounded">Guardar</button></header>
            <div className="flex-1 flex overflow-hidden">
                {sel && (
                    <div className="w-80 border-r p-4 space-y-4">
                        <input type="text" value={sel.name} onChange={e => upd({...sel, name: e.target.value})} className="w-full p-2 border rounded bg-transparent"/>
                        <select value={sel.shape} onChange={e => upd({...sel, shape: e.target.value as 'square' | 'round'})} className="w-full p-2 border rounded bg-transparent"><option value="square">Cuadrada</option><option value="round">Redonda</option></select>
                        <div className="grid grid-cols-2 gap-2"><input type="number" min="1" value={sel.width} onChange={e => upd({...sel, width: Number(e.target.value)})} className="p-2 border rounded bg-transparent"/><input type="number" min="1" value={sel.height} onChange={e => upd({...sel, height: Number(e.target.value)})} className="p-2 border rounded bg-transparent"/></div>
                        <button onClick={() => setZone(z => ({...z, tables: z.tables.filter(it => it.id !== selId)}))} className="w-full text-red-500">Borrar</button>
                    </div>
                )}
                <div className="flex-1 p-8 grid gap-4 overflow-auto" style={{ gridTemplateColumns: `repeat(${zone.cols}, 60px)`, gridTemplateRows: `repeat(${zone.rows}, 60px)` }}>
                    {zone.tables.map(t => <div key={t.id} onClick={() => setSelId(t.id)} className={`flex items-center justify-center border-2 ${selId === t.id ? 'border-emerald-500 bg-emerald-100' : 'border-gray-300'}`} style={{ gridRow: `${t.row} / span ${t.height}`, gridColumn: `${t.col} / span ${t.width}` }}>{t.name}</div>)}
                    {Array.from({ length: zone.rows * zone.cols }).map((_, i) => { const r = Math.floor(i/zone.cols)+1, c = (i%zone.cols)+1; if(isOccupied(r,c)) return null; return <div key={i} onClick={() => add(r,c)} className="border-2 border-dashed border-gray-200 flex items-center justify-center text-gray-300">+</div>; })}
                </div>
            </div>
        </div>
    );
...
const SettingsModal: React.FC<{ isOpen: boolean; onClose: () => void; onEditZoneLayout: (zone: Zone) => void; initialPage?: SettingsPage; }> = ({ isOpen, onClose, onEditZoneLayout, initialPage = 'general' }) => {
    ...
    const [zs, setZs] = useState<Zone[]>([]);
    useEffect(() => { if(isOpen) { getAppSettings().then(setSt); getZones().then(setZs); } }, [isOpen]);
    const save = async () => { if(st) { await saveAppSettings(st); alert("Guardado"); } };
    if (!isOpen || !st) return null;
    const nav = [
        { id: 'general', name: 'General', icon: <IconSettings /> },
        { id: 'store-data', name: 'Tienda', icon: <IconStore /> },
        { id: 'shipping-costs', name: 'Envío', icon: <IconDelivery /> },
        { id: 'payment-methods', name: 'Pago', icon: <IconPayment /> },
        { id: 'hours', name: 'Horarios', icon: <IconClock /> },
        { id: 'zones-tables', name: 'Mesas', icon: <IconTableLayout /> },
        { id: 'printing', name: 'Impresión', icon: <IconPrinter /> },
    ] as const;
    return (
        <div className="fixed inset-0 bg-black/60 z-40 flex justify-end">
            <div className="bg-white dark:bg-gray-900 h-full w-full max-w-5xl flex flex-col">
                <header className="p-4 border-b flex justify-between items-center"><h2>Configuración</h2><button onClick={onClose}><IconX /></button></header>
                <div className="flex-1 flex overflow-hidden">
                    <aside className="w-64 border-r p-4 space-y-1">{nav.map(n => <button key={n.id} onClick={() => setPage(n.id)} className={`w-full flex items-center gap-2 p-2 rounded ${page === n.id ? 'bg-emerald-50 text-emerald-600' : ''}`}>{n.icon}{n.name}</button>)}</aside>
                    <main className="flex-1 p-6 overflow-y-auto">
                        {page === 'general' && <GeneralSettings onSave={save} settings={st} setSettings={setSt} />}
                        {page === 'store-data' && <BranchSettingsView onSave={save} settings={st} setSettings={setSt} />}
                        {page === 'shipping-costs' && <ShippingSettingsView onSave={save} settings={st} setSettings={setSt} />}
                        {page === 'payment-methods' && <PaymentSettingsView onSave={save} settings={st} setSettings={setSt} />}
                        {page === 'hours' && <HoursSettings onSave={save} settings={st} setSettings={setSt} />}
                        {page === 'zones-tables' && <ZonesAndTablesSettings zones={zs} onAddZone={() => { const n = prompt("Nombre:"); if(n) saveZone({name:n, rows:5, cols:5}).then(()=>getZones().then(setZs)); }} onEditZoneName={z => saveZone(z).then(()=>getZones().then(setZs))} onDeleteZone={id => deleteZone(id).then(()=>getZones().then(setZs))} onEditZoneLayout={onEditZoneLayout} />}
                        {page === 'printing' && <PrintingSettingsView onSave={save} settings={st} setSettings={setSt} />}
                    </main>
                </div>
            </div>
        </div>
...
const QRModal: React.FC<{ isOpen: boolean; onClose: () => void; url: string; title: string; filename: string; }> = ({ isOpen, onClose, url, title }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 p-8 rounded-lg text-center space-y-6" onClick={e => e.stopPropagation()}>
                <h2 className="text-xl font-bold">{title}</h2>
                <img src={qr} alt="QR" className="w-64 h-64 mx-auto" />
                <button onClick={onClose} className="w-full bg-gray-200 p-2 rounded">Cerrar</button>
            </div>
        </div>
    );
};
...
const ShareView: React.FC<{ onGoToTableSettings: () => void }> = ({ onGoToTableSettings }) => {
    ...
    useEffect(() => { getAppSettings().then(setSt); getZones().then(setZs); }, []);
    if (!st) return <div className="p-10 text-center">Cargando...</div>;
    const menuLink = `${window.location.origin}${window.location.pathname}#/menu`;
    return (
        <div className="space-y-6">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border flex gap-4 items-center">
                <input type="text" readOnly value={menuLink} className="flex-1 p-2 bg-gray-100 rounded border-none"/>
                <button onClick={() => setQr({ open: true, url: menuLink, title: "Menú Digital" })} className="bg-emerald-600 text-white px-4 py-2 rounded">Ver QR</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {zs.map(z => (
                    <div key={z.id} className="bg-white dark:bg-gray-800 p-6 rounded-lg border">
                        <h3 className="font-bold mb-4">{z.name}</h3>
                        <div className="grid grid-cols-4 gap-2">{z.tables.map(t => <button key={t.id} onClick={() => setQr({ open: true, url: `${menuLink}?table=${t.name}&zone=${z.name}`, title: `Mesa ${t.name}` })} className="p-2 border rounded bg-gray-50">{t.name}</button>)}</div>
                    </div>
                ))}
            </div>
            {zs.length === 0 && <button onClick={onGoToTableSettings} className="text-emerald-600 underline">Configurar mesas</button>}
            <QRModal isOpen={qr.open} onClose={() => setQr({ ...qr, open: false })} url={qr.url} title={qr.title} filename="qr.png" />
        </div>
    );
};
