...
}> = ({ label, options, selectedOption, onSelect, icon }) => {
                    ...
                    <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                        {options.map(option => (
                            <button
                                ...
                                className={`block w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 ${selectedOption === option ? 'text-emerald-600 font-bold dark:text-emerald-400' : 'text-gray-700 dark:text-gray-200'}`}
                                role="menuitem"
                            >
                                {option}
                            </button>
...
const DashboardStatCard: React.FC<{ 
    ...
    trendValue?: string 
}> = ({ title, value, secondaryValue, trend, trendValue }) => (
...
const extractTipFromComments = (comment: string | undefined): number => {
    if (!comment) return 0;
    // Look for patterns like "Propina: $5.00" or "Propina: MXN 5.00"
    // Adjust Regex to match the format saved in CustomerView: "... | Propina: CODE X.XX"
    const match = comment.match(/Propina:\s*\w+\s*([\d.]+)/);
...
const Dashboard: React.FC = () => {
    ...
    // --- Filter Logic ---
    const { filteredOrders, previousPeriodOrders } = useMemo(() => {
        const now = new Date();
        ...
        switch (dateRange) {
            case 'Hoy':
                ...
                prevStartTime = new Date(startOfToday);
                prevStartTime.setDate(startOfToday.getDate() - 1);
            ...
            case 'Ayer':
                ...
                prevStartTime.setDate(startTime.getDate() - 1);
                prevEndTime = startTime;
                // End time for 'Yesterday' is startOfToday
                break;
            case 'Últimos 7 días':
                startTime = new Date(startOfToday);
                startTime.setDate(startOfToday.getDate() - 7);
                prevStartTime = new Date(startTime);
                prevStartTime.setDate(startTime.getDate() - 7);
                prevEndTime = startTime;
                break;
            case 'Últimos 30 días':
                ...
                prevStartTime = new Date(startTime);
                prevStartTime.setDate(startTime.getDate() - 30);
                prevEndTime = startTime;
                break;
            default:
                startTime = startOfToday; // Default to Today
                prevStartTime = new Date(startOfToday);
                prevStartTime.setDate(startOfToday.getDate() - 1);
                prevEndTime = startOfToday;
        }
        ...
        const current = orders.filter(o => {
            ...
            // Specific logic for "Ayer" to cap at end of day
            if (dateRange === 'Ayer') {
                return d >= startTime && d < startOfToday;
            }
            return d >= startTime;
        });
        ...
        const previous = orders.filter(o => {
            const d = new Date(o.createdAt);
            return d >= prevStartTime && d < prevEndTime;
        });

        // Apply Channel Filtering (OrderType)
        const filterByChannel = (list: Order[]) => {
            if (channelFilter === 'Todos') return list;
            // Map "Punto de venta" to logic (assuming DineIn/Takeaway handled manually are POS)
            // For now, let's map: 
            // "Domicilio" -> OrderType.Delivery
            // "Para Llevar" -> OrderType.TakeAway
            // "Mesa" -> OrderType.DineIn
            if (channelFilter === 'Domicilio') return list.filter(o => o.orderType === OrderType.Delivery);
            if (channelFilter === 'Para Llevar') return list.filter(o => o.orderType === OrderType.TakeAway);
            if (channelFilter === 'Mesa') return list.filter(o => o.orderType === OrderType.DineIn);
            return list;
        };

        return {
            filteredOrders: filterByChannel(current),
            previousPeriodOrders: filterByChannel(previous)
        };
    ...
    const stats = useMemo(() => {
        const calculateStats = (list: Order[]) => {
            ...
            const validOrders = list.filter(o => o.status !== OrderStatus.Cancelled);
            
            const totalSales = validOrders.reduce((sum, o) => sum + o.total, 0);
            const count = validOrders.length; // Count of non-cancelled orders
            const tips = validOrders.reduce((sum, o) => sum + extractTipFromComments(o.generalComments), 0);
            const deliveryCount = validOrders.filter(o => o.orderType === OrderType.Delivery).length;
            const pickupCount = validOrders.filter(o => o.orderType === OrderType.TakeAway).length;
            const dineInCount = validOrders.filter(o => o.orderType === OrderType.DineIn).length;

            return { totalSales, count, tips, deliveryCount, pickupCount, dineInCount };
        };

        const current = calculateStats(filteredOrders);
        const prev = calculateStats(previousPeriodOrders);

        // Trend calculation helper
        const getTrend = (curr: number, previous: number) => {
            if (previous === 0) return { dir: 'neutral', val: '' } as const;
            const diff = ((curr - previous) / previous) * 100;
            return {
        ...
        return {
            ...
            prev,
            trends: {
                sales: getTrend(current.totalSales, prev.totalSales),
                count: getTrend(current.count, prev.count),
            }
        };
    }, [filteredOrders, previousPeriodOrders]);

    const averageTicket = stats.current.count > 0 ? stats.current.totalSales / stats.current.count : 0;
    
    // Prepare Data for "Sales by Type" Bar Chart (CSS-based)
    const maxTypeCount = Math.max(stats.current.deliveryCount, stats.current.pickupCount, stats.current.dineInCount, 1);
    const getBarWidth = (val: number) => `${(val / maxTypeCount) * 100}%`;

    if (isLoading) {
        return <div className="p-10 text-center animate-pulse text-gray-500">Sincronizando movimientos...</div>;
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-4">
                ...
                <FilterDropdown 
                    label="Canal" 
                    options={['Todos', 'Domicilio', 'Para Llevar', 'Mesa']} 
                    selectedOption={channelFilter}
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <DashboardStatCard 
                    ...
                    value={`$${stats.current.totalSales.toFixed(2)}`} 
                    secondaryValue={stats.prev.totalSales > 0 ? `Prev: $${stats.prev.totalSales.toFixed(2)}` : 'Sin datos previos'} 
                    trend={stats.trends.sales.dir}
                    trendValue={stats.trends.sales.val}
                />
                <DashboardStatCard 
                    title="Pedidos Completados" 
                    value={stats.current.count.toString()} 
                    secondaryValue={stats.prev.count > 0 ? `Prev: ${stats.prev.count}` : 'Sin datos previos'}
                    trend={stats.trends.count.dir}
                    trendValue={stats.trends.count.val}
                />
                <DashboardStatCard 
                    title="Envíos Realizados" 
                    value={stats.current.deliveryCount.toString()} 
                    secondaryValue={`Pickup: ${stats.current.pickupCount} | Mesa: ${stats.current.dineInCount}`}
                />
                <DashboardStatCard 
                    title="Propinas Recibidas" 
                    value={`$${stats.current.tips.toFixed(2)}`} 
                    secondaryValue="Calculado de comentarios" 
                />
                ...
                <div className="bg-white dark:bg-gray-800 p-5 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 lg:col-span-2">
                    ...
                    <div className="h-32 flex flex-col items-center justify-center">
                        ...
                        <p className="text-sm text-gray-400 mt-2">Promedio por pedido en este periodo</p>
                    </div>
                </div>

                <div className="bg-white dark:bg-gray-800 p-5 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 lg:col-span-2">
                    <h4 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-4">Pedidos por Tipo</h4>
                    <div className="space-y-4 pt-2">
                        {/* Delivery Bar */}
                        <div className="flex items-center gap-3">
                            <div className="w-24 text-sm font-medium text-gray-600 dark:text-gray-300 flex items-center gap-2">
                                <IconDelivery className="h-4 w-4"/> Domicilio
                            </div>
                            <div className="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-4 overflow-hidden">
                                <div className="bg-blue-500 h-full rounded-full transition-all duration-1000" style={{ width: getBarWidth(stats.current.deliveryCount) }}></div>
                            </div>
                            <span className="text-sm font-bold w-8 text-right">{stats.current.deliveryCount}</span>
                        </div>
                        {/* Pickup Bar */}
                        <div className="flex items-center gap-3">
                            <div className="w-24 text-sm font-medium text-gray-600 dark:text-gray-300 flex items-center gap-2">
                            ...
                            <div className="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-4 overflow-hidden">
                                <div className="bg-purple-500 h-full rounded-full transition-all duration-1000" style={{ width: getBarWidth(stats.current.pickupCount) }}></div>
                            </div>
                            <span className="text-sm font-bold w-8 text-right">{stats.current.pickupCount}</span>
                        </div>
                        {/* DineIn Bar */}
                        <div className="flex items-center gap-3">
                            <div className="w-24 text-sm font-medium text-gray-600 dark:text-gray-300 flex items-center gap-2">
                                <IconTableLayout className="h-4 w-4"/> Mesa
                            </div>
                            <div className="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-4 overflow-hidden">
                                <div className="bg-orange-500 h-full rounded-full transition-all duration-1000" style={{ width: getBarWidth(stats.current.dineInCount) }}></div>
                            </div>
                            <span className="text-sm font-bold w-8 text-right">{stats.current.dineInCount}</span>
                        </div>
                    </div>
                </div>
            </div>
             <div className="fixed bottom-5 right-5 z-20">
                <button className="bg-gray-800 text-white px-4 py-2 rounded-lg shadow-lg flex items-center space-x-2 text-sm dark:bg-gray-700 dark:text-gray-200 hover:scale-105 transition-transform">
