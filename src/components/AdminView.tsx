...
// --- COMPONENTE TOAST VISUAL MEJORADO ---
const NewOrderToast: React.FC<{ order: Order | null; onClose: () => void }> = ({ order, onClose }) => {
    if (!order) return null;

    return (
        <div className="fixed top-24 right-5 z-50 w-80 animate-[slideInRight_0.5s_ease-out]">
            <div className="relative bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border-l-4 border-emerald-500 shadow-2xl rounded-r-lg p-4 overflow-hidden border-y border-r border-gray-200 dark:border-gray-700">
                ...
                <div className="flex justify-between items-start relative z-10">
                    <div className="flex items-start gap-3">
                         ...
                         <div>
                             <h4 className="font-bold text-gray-900 dark:text-gray-100 text-base">¡Nuevo Pedido Recibido!</h4>
                             <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mt-1">{order.customer.name}</p>
                             <p className="text-xs text-gray-500 dark:text-gray-400 font-mono mt-0.5">#{order.id.slice(0,6)} • ${order.total.toFixed(2)}</p>
                         </div>
                    </div>
