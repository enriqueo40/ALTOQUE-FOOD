...
const ProductModal: React.FC<{ isOpen: boolean; onClose: () => void; onSave: (product: Omit<Product, 'id' | 'created_at'> & { id?: string }) => void; product: Product | null; categories: Category[]; personalizations: Personalization[] }> = ({ isOpen, onClose, onSave, product, categories, personalizations }) => {
    ...
    const [selectedPersIds, setSelectedPersIds] = useState<string[]>([]);
    ...
    useEffect(() => {
        if (isOpen) {
            if (product) {
                setFormData(product);
                setSelectedPersIds(product.personalizationIds || []);
            } else {
                setFormData({
                    name: '',
                    description: '',
    ...
    const togglePersonalization = (id: string) => {
        setSelectedPersIds(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const finalProduct = {
            ...
            personalizationIds: selectedPersIds
        };
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg max-h-full overflow-y-auto">
                <form onSubmit={handleSubmit} className="p-8">
                    ...
                    <div className="space-y-4">
                        ...
                        {/* Personalizations Linker */}
                        <div>
                             <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 uppercase tracking-tighter">Personalizaciones vinculadas</label>
                             <div className="flex flex-wrap gap-2">
                                 {personalizations.map(p => (
                                     <button
                                        type="button"
                                        key={p.id}
                                        onClick={() => togglePersonalization(p.id)}
                                        className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all border ${selectedPersIds.includes(p.id) ? 'bg-emerald-600 text-white border-emerald-600 shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 border-transparent hover:border-gray-300'}`}
                                     >
                                         {p.name}
                                     </button>
                                 ))}
                             </div>
                             {personalizations.length === 0 && <p className="text-xs text-gray-500 italic">No hay personalizaciones creadas.</p>}
                        </div>
                    ...
                    <div className="flex justify-end space-x-4 pt-6">
                        ...
                        <button type="submit" className="px-4 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 font-bold">Guardar Producto</button>
                    </div>
                </form>
            </div>
        </div>
...
const ProductsView: React.FC = () => {
    ...
    const handleSaveProduct = async (productData: Omit<Product, 'id' | 'created_at'> & { id?: string }) => {
        try {
            ...
            if (productData.personalizationIds) {
                 await updateProductPersonalizationLinks(saved.id, productData.personalizationIds);
            }
    ...
    return (
        <div>
            <div className="flex justify-end items-center mb-6 gap-x-4">
                ...
                <button onClick={() => handleOpenProductModal(null)} className="px-4 py-2 bg-emerald-600 text-white rounded-md text-sm font-semibold hover:bg-emerald-700 font-bold">
                    + Nuevo producto
                </button>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 h-full w-full max-w-3xl flex flex-col relative">
                ...
                <div className="flex flex-1 overflow-hidden">
                    <form onSubmit={handleSubmit} className="flex-1 flex flex-col lg:w-2/3">
                        ...
                        <footer className="p-6 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 flex justify-end items-center space-x-3 sticky bottom-0 shrink-0">
                            <button type="button" onClick={onClose} className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">Cancelar</button>
                            <button type="submit" className="px-4 py-2 bg-emerald-600 text-white rounded-md text-sm font-semibold hover:bg-emerald-700 font-bold">Agregar promoción</button>
                        </footer>
                    </form>
                    ...
                    <div>
                     ...
                     <div className="relative group">
                        ...
                        <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 shadow-xl rounded-lg border dark:border-gray-700 hidden group-hover:block z-10 overflow-hidden">
                             <button onClick={handleCopyOrder} className="w-full text-left px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2 text-sm font-bold"><IconDuplicate className="h-4 w-4"/> Copiar detalles</button>
                             <button onClick={() => { onUpdateStatus(order.id, OrderStatus.Cancelled); handleClose(); }} className="w-full text-left px-4 py-3 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 dark:text-red-400 flex items-center gap-2 text-sm border-t dark:border-gray-700 font-bold"><IconX className="h-4 w-4"/> Cancelar pedido</button>
                        </div>
                    </div>
                </div>
                ...
                <div className="w-3/5 flex flex-col border-r dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                    <div className="p-4 border-b dark:border-gray-700 bg-white dark:bg-gray-800 flex gap-3">
                         <div className="relative flex-1">
                            ...
                            <input 
                                ...
                                className="w-full pl-10 pr-4 py-2.5 rounded-lg border dark:border-gray-600 bg-gray-100 dark:bg-gray-700 focus:ring-2 focus:ring-emerald-500 outline-none font-bold"
                            />
                         </div>
                    </div>
                    ...
                    <div className="flex gap-2 overflow-x-auto p-2 border-b dark:border-gray-700 bg-white dark:bg-gray-800">
                        <button 
                            ...
                            className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap ${activeCategory === 'all' ? 'bg-emerald-600 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}
                        >
                            Todo
                        </button>
                        {categories.map(cat => (
                            <button 
                                ...
                                className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap ${activeCategory === cat.id ? 'bg-emerald-600 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}
                            >
                                {cat.name}
                            </button>
                        ))}
                    ...
                    <div className="p-4 space-y-4 border-b dark:border-gray-700">
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full p-2 border dark:border-gray-700 rounded bg-gray-50 dark:bg-gray-800 focus:ring-2 focus:ring-emerald-500 outline-none font-medium"
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button 
                                ...
                                className={`p-2 rounded-lg border text-sm font-bold flex items-center justify-center gap-2 ${orderType === OrderType.TakeAway ? 'bg-emerald-50 border-emerald-500 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300' : 'dark:border-gray-700 text-gray-500'}`}
                            >
                                <IconStore className="h-4 w-4"/> Para Llevar
                            </button>
                            <button 
                                onClick={() => setOrderType(OrderType.DineIn)}
                                className={`p-2 rounded-lg border text-sm font-bold flex items-center justify-center gap-2 ${orderType === OrderType.DineIn ? 'bg-emerald-50 border-emerald-500 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300' : 'dark:border-gray-700 text-gray-500'}`}
                            >
                        ...
                        {orderType === OrderType.DineIn && (
                            <select 
                                ...
                                className="w-full p-2 border dark:border-gray-700 rounded bg-gray-50 dark:bg-gray-800 outline-none text-sm font-bold"
                            >
                                ...
                                <div key={item.cartItemId} className="flex justify-between items-center bg-gray-50 dark:bg-gray-800/50 p-2 rounded-lg border dark:border-gray-700">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-gray-500 font-bold">${item.price.toFixed(2)}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                    ...
                    <div className="p-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                        ...
                        <div className="flex items-center justify-between mb-4 p-3 bg-white dark:bg-gray-700 rounded-lg border dark:border-gray-600">
                            ...
                            <button 
                                ...
                                className={`px-3 py-1 rounded-full text-xs font-black uppercase transition-colors ${paymentStatus === 'paid' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-yellow-100 text-yellow-700 border border-yellow-200'}`}
                            >
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <button onClick={handleCreateOrder} className="px-4 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-black shadow-lg shadow-emerald-900/20">
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
                ...
                return (
                    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900 p-4">
                        ...
                        <div className="flex justify-end gap-3 mb-4">
                            <button className="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 font-bold">
                                Ver uso de suscripción
                            </button>
                            <button className="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2 font-bold">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6 flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-gray-200 dark:divide-gray-700">
                            <div className="p-4 flex items-center justify-center md:justify-start md:w-48">
                                <button className="flex items-center gap-2 text-gray-700 dark:text-gray-200 font-black px-3 py-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors uppercase tracking-tight">
                            ...
                            <div className="flex-1 grid grid-cols-2 md:grid-cols-4 divide-x divide-gray-200 dark:divide-gray-700">
                                <div className="p-4 text-center">
                                    <p className="text-2xl font-black text-gray-800 dark:text-gray-100">{tableStats.requestingBill}</p>
                                    <p className="text-xs text-gray-500 uppercase tracking-wide mt-1 font-bold">mesas solicitando cuenta</p>
                                </div>
                                <div className="p-4 text-center">
                                    <p className="text-2xl font-black text-gray-800 dark:text-gray-100">{tableStats.requestingWaiter}</p>
                                    <p className="text-xs text-gray-500 uppercase tracking-wide mt-1 font-bold">mesas solicitando mesero</p>
                                </div>
                                <div className="p-4 text-center">
                                    <p className="text-2xl font-black text-gray-800 dark:text-gray-100">{tableStats.pendingOrders}</p>
                                    <p className="text-xs text-gray-500 uppercase tracking-wide mt-1 font-bold">mesas con comandas pendientes</p>
                                </div>
                                <div className="p-4 text-center">
                                    <p className="text-2xl font-black text-gray-800 dark:text-gray-100">{tableStats.activeTables}</p>
                                    <p className="text-xs text-gray-500 uppercase tracking-wide mt-1 font-bold">mesas activas</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="flex gap-2 overflow-x-auto pb-2">
                                {zones.map(zone => (
                                    <button
                                        ...
                                        className={`px-6 py-2 rounded-lg border font-black text-sm transition-all whitespace-nowrap uppercase tracking-wider ${
                                ...
                                {zones.length === 0 && (
                                     <div className="text-sm text-gray-500 p-2 italic">No hay zonas configuradas. Ve a Configuración &gt; Zonas y mesas.</div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex-1 p-8 overflow-auto relative min-h-[400px]">
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`
                                                    ...
                                                    ${isOccupied 
                                                        ? 'bg-red-50 border-red-400 dark:bg-red-900/20 dark:border-red-500/50 shadow-md scale-105' 
                                                        : 'bg-gray-50 border-gray-200 dark:bg-gray-700/50 dark:border-gray-600 hover:border-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20'
                                                    }
                                                `}
                                            >
                                                <span className={`text-2xl font-black ${isOccupied ? 'text-red-500 dark:text-red-400' : 'text-gray-400 dark:text-gray-500'}`}>
                                                    {table.name}
                                                </span>
                                                
                                                {isOccupied && order && (
                                                    <div className="absolute -top-2 -right-2">
                                                        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white text-xs font-black shadow-lg">
                                                ...
                                                {isOccupied && (
                                                    <div className="mt-1 px-2 py-0.5 bg-white dark:bg-gray-800 rounded text-[10px] font-black text-gray-600 dark:text-gray-300 shadow-sm border border-gray-100 dark:border-gray-700 truncate max-w-[90%] uppercase">
                                                        Ocupada
                                                    </div>
                                                )}
                                            </div>
                            ...
                            <div className="absolute bottom-4 right-4">
                                <div className="bg-gray-900 text-white px-4 py-2 rounded-lg shadow-lg text-sm font-black flex items-center gap-2 cursor-pointer hover:bg-gray-800 uppercase tracking-tighter">
            ...
            case 'comandas-digitales':
                 return <div className="text-center p-20 text-gray-400 bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 font-bold uppercase tracking-widest opacity-30">Comandas digitales (próximamente)</div>;
            default:
                return null;
        }
    };
                        ...
                        <button
                            ...
                            className={`${
                                activeTab === tab.id
                                    ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400'
                                    : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:border-gray-600'
                            } whitespace-nowrap py-4 px-1 border-b-2 font-black text-sm uppercase tracking-tight`}
                        >
                            {tab.title}
                        </button>
                    ))}
...
const ShippingSettingsView: React.FC<{ onSave: () => Promise<void>; settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }> = ({ onSave, settings, setSettings }) => {
    ...
    const handleCancel = () => { setSettings(prev => ({...prev, shipping: originalSettings})); };

    const inputClasses = "w-24 text-center px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm font-bold";
    const fullInputClasses = "mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm";
        ...
        <div className="space-y-6">
            <SettingsCard title="Tipo de costo de envío" onSave={onSave} onCancel={handleCancel}>
                <select 
                    ...
                    className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm font-bold"
                >
                   {Object.values(ShippingCostType).map(type => <option key={type} value={type}>{type}</option>)}
                </select>

                {/* MODIFICACIÓN: Campo de entrada para costo fijo dinámico */}
                {settings.shipping.costType === ShippingCostType.Fixed && (
                    <div className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-lg animate-in fade-in slide-in-from-top-1">
                        <label className="block text-sm font-black text-emerald-800 dark:text-emerald-300 mb-2 uppercase tracking-wider">Costo fijo de envío ($)</label>
                        <div className="relative max-w-[200px]">
                            <span className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-emerald-600 dark:text-emerald-400 font-black">$</span>
                            <input 
                                type="number" 
                                step="0.01"
                                value={settings.shipping.fixedCost || 0} 
                                onChange={e => setSettings(p => ({...p, shipping: {...p.shipping, fixedCost: parseFloat(e.target.value) || 0}}))} 
                                className={`${fullInputClasses} pl-8 font-black text-xl text-emerald-700 dark:text-emerald-400`}
                                placeholder="0.00"
                            />
                        </div>
                        <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-2 font-bold italic">Este monto se sumará automáticamente al total de pedidos a domicilio en el menú online.</p>
                    </div>
                )}
                
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 italic font-medium">Gestiona cuánto cobras por cada entrega según el tipo de servicio.</p>
                ...
                <div className="mt-4 space-y-3 pt-4 border-t dark:border-gray-700">
                    <label className="flex items-center">
                        ...
                        <span className="ml-2 text-sm font-bold">Envío gratis si se alcanza una compra mínima</span>
                    </label>
                     <label className="flex items-center">
                        <input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500" />
                        <span className="ml-2 text-sm font-bold">Se requiere una compra mínima para habilitar envíos</span>
                    </label>
                </div>
            </SettingsCard>

             <SettingsCard title="Tiempo para pedidos a domicilio" onSave={onSave} onCancel={handleCancel}>
                <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Información visible para el cliente desde su pedido.</p>
                <div className="flex items-center gap-4 mt-4">
                    <div className="flex items-center gap-2">
                         ...
                         <span className="text-xs font-bold uppercase tracking-tighter">mins</span>
                    </div>
                     <span className="text-gray-400 font-black">HASTA</span>
                    <div className="flex items-center gap-2">
                 ...
                 <h4 className="font-black mt-8 uppercase text-xs tracking-widest text-gray-400">Tiempo para recoger</h4>
...
// ... Rest of AdminView (Keeping consistency with provided file structure)
const SearchableDropdown: React.FC<{ options: Currency[], selected: Currency, onSelect: (option: Currency) => void }> = ({ options, selected, onSelect }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button type="button" onClick={() => setIsOpen(!isOpen)} className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500 sm:text-sm font-bold">
                ...
                <div className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                    <div className="p-2">
                         <input
                            ...
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-transparent focus:ring-emerald-500 focus:border-emerald-500 font-medium"
                        />
                    </div>
                    {filteredOptions.map(option => (
                        <button
                            ...
                            className="w-full text-left cursor-default select-none relative py-2 pl-10 pr-4 text-gray-900 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-emerald-900/20"
                        >
                            <span className={`font-bold block truncate ${selected.code === option.code ? 'text-emerald-600' : ''}`}>{option.name}</span>
                        </button>
                    ))}
                </div>
            )}
...
const GeneralSettings: React.FC<{ onSave: () => Promise<void>; settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }> = ({ onSave, settings, setSettings }) => {
    ...
    const handleCancel = () => { setSettings(prev => ({...prev, company: originalSettings})); }

    return (
        <div className="space-y-6">
             <SettingsCard title="Datos de empresa" onSave={onSave} onCancel={handleCancel}>
                 <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-1">Nombre de empresa</label>
                 <input type="text" value={settings.company.name} onChange={e => setSettings(p => ({...p, company: {...p.company, name: e.target.value}}))} className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm font-bold"/>

                 <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mt-6 mb-1">Divisa de facturación</label>
                 <p className="text-xs text-gray-500 dark:text-gray-400 mb-2 italic">Escoge la divisa que tus clientes verán en el menú online.</p>
...
const BranchSettingsView: React.FC<{ onSave: () => Promise<void>; settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }> = ({ onSave, settings, setSettings }) => {
    ...
    const handleCancel = () => { setSettings(prev => ({...prev, branch: originalSettings})); };
    ...
    const inputClasses = "mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm font-bold";

    return (
        <div className="space-y-6">
            <SettingsCard title="Datos de sucursal" onSave={onSave} onCancel={handleCancel}>
                <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-1">Alias de sucursal</label>
                <input 
                    type="text" 
                ...
                />
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 italic">Nombre corto para identificar esta sucursal (ej: Centro, Las Américas).</p>

                 <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mt-6 mb-1">Dirección completa</label>
                <input 
                    type="text" 
                ...
                <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mt-6 mb-1">Ubicación en Google Maps</label>
                {settings.branch.googleMapsLink ? (
                    <div className="flex items-center gap-2 mt-1">
                        <a href={settings.branch.googleMapsLink} target="_blank" rel="noopener noreferrer" className="text-emerald-600 dark:text-emerald-400 text-sm underline truncate font-bold">{settings.branch.googleMapsLink}</a>
                        <button onClick={() => setSettings(p => ({...p, branch: {...p.branch, googleMapsLink: ''}}))} className="text-red-500 p-2 hover:bg-red-50 rounded-full"><IconTrash className="h-4 w-4"/></button>
                    </div>
                ) : (
                    <button type="button" onClick={handleGetLocation} className="mt-1 flex items-center gap-2 px-4 py-2 border border-emerald-300 dark:border-emerald-700 rounded-md text-sm font-bold text-emerald-700 dark:text-emerald-300 hover:bg-emerald-50 dark:hover:bg-emerald-900/20">
            ...
            <SettingsCard title="Número de WhatsApp para pedidos" noActions>
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 italic">El número al que llegarán las comandas de los pedidos online.</p>
                <div className="flex justify-between items-center p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-100 dark:border-emerald-800">
                    <span className="font-black text-emerald-800 dark:text-emerald-200 text-lg tracking-wider">{settings.branch.whatsappNumber}</span>
                    <button type="button" className="px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md text-xs font-black uppercase hover:bg-gray-50 transition-colors shadow-sm">Cambiar</button>
                </div>
            </SettingsCard>
        </div>
    );
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <main className="flex-1 overflow-auto p-8 scroll-smooth">
                    <div className="max-w-7xl mx-auto">
                        ...
                        {currentPage === 'share' && <ShareView onGoToTableSettings={() => setIsSettingsOpen(true)}/>}
                    </div>
                </main>
            </div>
