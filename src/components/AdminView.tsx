...
const PromotionModal: React.FC<{
    ...
    const handleSelectAllFiltered = () => {
        ...
        setFormData(prev => {
            ...
            const uniqueIds = new Set([...prev.productIds, ...filteredIds]);
            return { ...prev, productIds: Array.from(uniqueIds) };
        });
    };
    ...
    const handleDeselectAllFiltered = () => {
        ...
        setFormData(prev => ({
            ...prev,
            productIds: prev.productIds.filter(id => !filteredIds.has(id))
        }));
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const finalPromo = {
            ...formData,
            id: promotion?.id,
            // Ensure we don't send empty strings if any crept in
                ...
                <div className="flex flex-1 overflow-hidden">
                                ...
                                <div className="border dark:border-gray-700 rounded-md overflow-hidden bg-white dark:bg-gray-800">
                                    <div className="p-3 bg-gray-50 dark:bg-gray-900/50 border-b dark:border-gray-700">
                                        <div className="relative">
                                            ...
                                            <input 
                                                ...
                                                className="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 focus:outline-none focus:ring-1 focus:ring-emerald-500 dark:text-white"
                                            />
                                        </div>
                                        <div className="flex justify-between items-center mt-2">
                                            <span className="text-xs text-gray-500 dark:text-gray-400">
                                                {formData.productIds.length} seleccionados
                                            </span>
                                            <div className="flex gap-2">
                                                <button type="button" onClick={handleSelectAllFiltered} className="text-xs text-emerald-600 hover:text-emerald-700 font-medium">Seleccionar visibles</button>
                                                <button type="button" onClick={handleDeselectAllFiltered} className="text-xs text-red-600 hover:text-red-700 font-medium">Deseleccionar visibles</button>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                                        {products
                                            ...
                                            .map(product => (
                                                <label key={product.id} className="flex items-center space-x-3 p-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded cursor-pointer transition-colors">
                                                    <input
                                                        type="checkbox"
                                                        checked={formData.productIds.includes(product.id)}
                                                        onChange={() => toggleProductSelection(product.id)}
                                                        className="h-4 w-4 text-emerald-600 rounded border-gray-300 focus:ring-emerald-500 dark:bg-gray-700 dark:border-gray-600"
                                                    />
                                                    <div className="flex items-center gap-3 flex-1">
                                                        <img src={product.imageUrl} alt="" className="w-8 h-8 rounded object-cover bg-gray-200" />
                                                        <div className="flex-1">
                                                            <p className="text-sm font-medium text-gray-700 dark:text-gray-200">{product.name}</p>
                                        ...
                                        {products.filter(p => p.name.toLowerCase().includes(productSearch.toLowerCase())).length === 0 && (
                                            <div className="text-center py-4 text-sm text-gray-500">No hay productos que coincidan.</div>
                                        )}
                                    </div>
                                </div>
                            )}
                    ...
                    <div className="w-1/3 bg-gray-100 dark:bg-gray-900/50 p-6 border-l dark:border-gray-700 hidden lg:block overflow-y-auto">
                         ...
                         <p className="text-sm text-gray-500 dark:text-gray-400 mt-4 mb-4">
                             {formData.discountType === DiscountType.Percentage ? `${formData.discountValue}%` : `$${formData.discountValue}`} de descuento en:
                         </p>
                         
                         {formData.appliesTo === PromotionAppliesTo.AllProducts ? (
                             <div className="p-3 bg-emerald-100 dark:bg-emerald-900/20 text-emerald-800 dark:text-emerald-200 rounded-md text-sm font-medium">
                                 Todos los productos
                             </div>
                         ) : (
                             <div className="space-y-2">
                                 {formData.productIds.length === 0 ? (
                                     <p className="text-xs text-gray-400 italic">Ningún producto seleccionado</p>
                                 ) : (
                                     products
                                        .filter(p => formData.productIds.includes(p.id))
                                        .map(p => (
                                            <div key={p.id} className="flex items-center gap-2 p-2 bg-white dark:bg-gray-800 rounded border dark:border-gray-700 text-sm">
                                                <img src={p.imageUrl} className="w-6 h-6 rounded object-cover" alt="" />
        ...
        <div className="space-y-6">
            ...
            <div className="border-b border-gray-200 dark:border-gray-700">
                <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                    ...
                    <button onClick={() => setActiveTab('mesas')} className={`${activeTab === 'mesas' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400' : 'border