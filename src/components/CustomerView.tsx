...
export default function CustomerView() {
    ...
    if (isLoading) return <div className="h-screen flex items-center justify-center bg-[#0a0b0d]"><div className="w-10 h-10 border-4 border-purple-500 border-t-transparent rounded-full animate-spin"></div></div>;
        ...
        <div className="min-h-screen bg-[#0a0b0d] pb-32">
            <header className="bg-[#15171d] p-4 sticky top-0 z-[60] border-b border-gray-800 shadow-xl">
                ...
                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                    <button onClick={() => setSelectedCategory('all')} className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase transition-all duration-300 ${selectedCategory === 'all' ? 'bg-purple-600 text-white shadow-lg' : 'bg-gray-800/50 text-gray-500'}`}>Todo</button>
                    {allCategories.map(cat => <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase transition-all duration-300 ${selectedCategory === cat.id ? 'bg-purple-600 text-white shadow-lg' : 'bg-gray-800/50 text-gray-500'}`}>{cat.name}</button>)}
                </div>
            </header>
                ...
                {allProducts.filter(p => p.available && (selectedCategory === 'all' || p.categoryId === selectedCategory)).map(product => (
                    <div key={product.id} onClick={() => setSelectedProduct(product)} className="bg-[#15171d] rounded-[2rem] p-3 flex gap-4 cursor-pointer border border-gray-800 transition-all active:scale-95 group">
                        ...
                        <div className="flex-1 flex flex-col justify-between py-1">
                            ...
                            <div className="flex justify-between items-center"><span className="font-black text-purple-400 text-lg tracking-tighter">${product.price.toFixed(2)}</span><div className="bg-purple-500/10 p-2 rounded-xl text-purple-400 border border-purple-500/20"><IconPlus className="h-5 w-5"/></div></div>
                        </div>
                    </div>
                ))}
            </div>
            {itemCount > 0 && <div className="fixed bottom-6 left-6 right-6 z-50"><button onClick={() => setView('cart')} className="w-full bg-purple-600 text-white p-5 rounded-[2rem] font-black flex justify-between shadow-2xl active:scale-95 transition-all uppercase text-[11px] tracking-[0.2em] border border-purple-400/20"><span>Ver Pedido ({itemCount})</span><span>Total: ${cartTotal.toFixed(2)}</span></button></div>}
            <Chatbot />
        </div>
    );
    ...
    if (view === 'cart') return (
        <div className="min-h-screen bg-[#0a0b0d] flex flex-col">
            ...
            <div className="flex-1 p-4 space-y-4 overflow-y-auto no-scrollbar">
                {cartItems.map(item => (
                    <div key={item.cartItemId} className="bg-[#15171d] p-4 rounded-[2rem] flex gap-4 border border-gray-800 shadow-xl"><img src={item.imageUrl} className="w-20 h-20 rounded-2xl object-cover" alt={item.name} /><div className="flex-1 flex flex-col justify-center"><h4 className="font-bold text-white text-sm">{item.name}</h4><p className="text-purple-400 font-black mt-1">${(item.price * item.quantity).toFixed(2)}</p><div className="flex items-center gap-5 mt-3"><button onClick={() => updateQuantity(item.cartItemId, item.quantity - 1)} className="w-9 h-9 flex items-center justify-center bg-gray-800 rounded-xl text-white"><IconMinus className="h-4 w-4"/></button><span className="font-black text-white text-lg">{item.quantity}</span><button onClick={() => updateQuantity(item.cartItemId, item.quantity + 1)} className="w-9 h-9 flex items-center justify-center bg-purple-600/20 text-purple-400 rounded-xl"><IconPlus className="h-4 w-4"/></button></div></div><button onClick={() => removeFromCart(item.cartItemId)} className="self-start mt-2 p-2 text-gray-600 hover:text-red-500"><IconTrash className="h-5 w-5"/></button></div>
                ))}
            </div>
            <div className="p-6 bg-[#15171d] border-t border-gray-800 rounded-t-[3rem] shadow-2xl"><button onClick={() => setView('checkout')} className="w-full bg-purple-600 text-white py-5 rounded-[2rem] font-black shadow-xl uppercase tracking-[0.2em] text-[11px] border border-purple-400/20">Ir al Pago</button></div>
        </div>
    );
                ...
                <div className="p-4 pb-48 space-y-6 max-w-xl mx-auto overflow-x-hidden">
                    <div className="bg-[#15171d] p-6 rounded-[2.5rem] border border-gray-800 shadow-2xl">
                        <div className="grid grid-cols-2 gap-3">
                            <button onClick={() => setOrderType(OrderType.Delivery)} className={`flex flex-col items-center justify-center p-5 rounded-[1.8rem] border-2 transition-all duration-500 gap-2 ${orderType === OrderType.Delivery ? 'bg-purple-600/10 border-purple-500 shadow-lg' : 'bg-[#0a0b0d] border-gray-800 opacity-50'}`}><IconStore className={`h-8 w-8 ${orderType === OrderType.Delivery ? 'text-purple-400' : 'text-gray-600'}`} /><span className={`text-[10px] font-black uppercase tracking-widest ${orderType === OrderType.Delivery ? 'text-white' : 'text-gray-600'}`}>Domicilio</span></button>
                            <button onClick={() => setOrderType(OrderType.TakeAway)} className={`flex flex-col items-center justify-center p-5 rounded-[1.8rem] border-2 transition-all duration-500 gap-2 ${orderType === OrderType.TakeAway ? 'bg-purple-600/10 border-purple-500 shadow-lg' : 'bg-[#0a0b0d] border-gray-800 opacity-50'}`}><IconLocationMarker className={`h-8 w-8 ${orderType === OrderType.TakeAway ? 'text-purple-400' : 'text-gray-600'}`} /><span className={`text-[10px] font-black uppercase tracking-widest ${orderType === OrderType.TakeAway ? 'text-white' : 'text-gray-600'}`}>Llevar</span></button>
                        </div>
                    </div>
                    <div className="bg-[#15171d] p-6 rounded-[2.5rem] border border-gray-800 shadow-2xl space-y-4">
                        <input type="text" placeholder="Tu Nombre" value={customerName} onChange={e => setCustomerName(e.target.value)} className="w-full p-4 bg-[#0a0b0d] border border-gray-800 rounded-2xl outline-none focus:border-purple-500 transition-all text-sm text-white" />
                        <input type="tel" placeholder="WhatsApp" value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} className="w-full p-4 bg-[#0a0b0d] border border-gray-800 rounded-2xl outline-none focus:border-purple-500 transition-all font-mono text-sm text-white" />
                    </div>
                    <div className="bg-[#15171d] p-6 rounded-[2.5rem] border border-gray-800 shadow-2xl">
                        <div className="grid grid-cols-2 gap-2">
                            {(settings?.payment[orderType === OrderType.Delivery ? 'deliveryMethods' : 'pickupMethods'] || ['Efectivo', 'Pago Móvil']).map(pm => (
                                <button key={pm} onClick={() => { setSelectedPaymentMethod(pm); setPaymentProof(null); }} className={`p-4 rounded-2xl border-2 font-bold text-[10px] uppercase transition-all duration-300 ${selectedPaymentMethod === pm ? 'bg-purple-600 border-purple-500 text-white shadow-lg' : 'bg-[#0a0b0d] border-gray-800 text-gray-600 hover:border-gray-700'}`}>{pm}</button>
                            ))}
                        </div>
                        {isDigital && (
                            <div className="mt-8 pt-8 border-t border-gray-800 space-y-6">
                                <div className="p-6 bg-purple-900/10 rounded-[2rem] border-2 border-purple-500/20 shadow-inner">
                                    <p className="text-purple-400 font-black text-[9px] uppercase tracking-[0.4em] mb-4">Datos Bancarios</p>
                                    <p className="text-white text-sm font-bold">Banco: {settings?.payment.pagoMovil?.bank || 'Consultar'}</p>
                                    <p className="text-purple-400 font-black text-lg">Tel: {settings?.payment.pagoMovil?.phone || 'Consultar'}</p>
                                </div>
                                <div className="space-y-4">
                                    {!paymentProof ? (
                                        <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-purple-500/30 rounded-[2.5rem] bg-[#0a0b0d] cursor-pointer shadow-2xl"><IconUpload className="h-8 w-8 text-purple-500 mb-3"/><span className="text-[10px] text-gray-400 font-black uppercase">Subir Capture</span><input type="file" className="hidden" accept="image/*" onChange={handleProofUpload} /></label>
                                    ) : (
                                        <div className="relative rounded-[2.5rem] overflow-hidden shadow-2xl border-2 border-purple-500"><img src={paymentProof} className="w-full h-64 object-cover opacity-80" alt="Recibo" /><button onClick={() => setPaymentProof(null)} className="absolute top-4 right-4 bg-red-600 text-white p-4 rounded-full shadow-2xl"><IconTrash className="h-5 w-5"/></button></div>
                                    )}
                                </div>
                            </div>
                        )}
