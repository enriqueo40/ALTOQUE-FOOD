...
import { IconPlus, IconMinus, IconClock, IconShare, IconArrowLeft, IconTrash, IconX, IconWhatsapp, IconTableLayout, IconSearch, IconLocationMarker, IconStore, IconUpload, IconCheck } from '../constants';
...
export default function CustomerView() {
    ...
    const handlePlaceOrder = async (customer: Customer, paymentMethod: PaymentMethod, paymentProof?: string) => {
        if (!settings) return;
        ...
        const newOrder: Omit<Order, 'id' | 'createdAt'> = {
            ...
            paymentProof: paymentProof // Attach base64 string
        };
        ...
        if (orderType === OrderType.DineIn) {
            messageParts = [
                ...
                paymentProof ? `*(Comprobante adjunto en el sistema)*` : ''
            ];
...
// ... (Header, RestaurantHero, MenuList, ProductRow, ProductDetailModal, CartSummaryView components remain unchanged) ...
...
const CheckoutView: React.FC<{
    cartTotal: number, 
    onPlaceOrder: (customer: Customer, paymentMethod: PaymentMethod, paymentProof?: string) => void, 
    settings: AppSettings, 
    orderType: OrderType 
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            if (file.size > 2 * 1024 * 1024) { // 2MB Limit
                alert("La imagen es muy pesada. Intenta con una imagen menor a 2MB.");
                return;
            }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if ((selectedPaymentMethod === 'Pago Móvil' || selectedPaymentMethod === 'Transferencia' || selectedPaymentMethod === 'Zelle') && !paymentProof) {
            alert("Por favor, adjunta el comprobante de pago (capture).");
            return;
        }
        onPlaceOrder(customer, selectedPaymentMethod, paymentProof);
    };
                                ...
                                <div className="mt-2 mb-4 animate-fade-in">
                                    {method === 'Pago Móvil' && settings.payment.pagoMovil && (
                                        <div className="p-4 bg-blue-900/20 border border-blue-800 rounded-lg text-sm text-blue-200">
                                            <p className="font-bold mb-1">Datos Pago Móvil:</p>
                                            <p>Banco: <span className="font-mono text-white">{settings.payment.pagoMovil.bank}</span></p>
                                            <p>Teléfono: <span className="font-mono text-white">{settings.payment.pagoMovil.phone}</span></p>
                                            <p>C.I./RIF: <span className="font-mono text-white">{settings.payment.pagoMovil.idNumber}</span></p>
                                        </div>
                                    )}
                                    {method === 'Transferencia' && settings.payment.transfer && (
                                        <div className="p-4 bg-blue-900/20 border border-blue-800 rounded-lg text-sm text-blue-200">
                                            <p className="font-bold mb-1">Datos Transferencia:</p>
                                            <p>Banco: <span className="font-mono text-white">{settings.payment.transfer.bank}</span></p>
                                            <p>Cuenta: <span className="font-mono text-white break-all">{settings.payment.transfer.accountNumber}</span></p>
                                            <p>Titular: <span className="font-mono text-white">{settings.payment.transfer.accountHolder}</span></p>
                                            <p>C.I./RIF: <span className="font-mono text-white">{settings.payment.transfer.idNumber}</span></p>
                                        </div>
                                    )}
                                    {(method === 'Pago Móvil' || method === 'Transferencia' || method === 'Zelle') && (
                                        <div className="mt-3">
                                            <label className="block text-sm font-bold text-gray-400 mb-2">Adjuntar Capture / Comprobante</label>
                                            <div className="relative">
                                                <input type="file" accept="image/*" onChange={handleFileUpload} className="hidden" id="payment-proof-upload" />
                                                <label htmlFor="payment-proof-upload" className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-600 border-dashed rounded-lg cursor-pointer bg-gray-800 hover:bg-gray-700 hover:border-gray-500 transition-colors">
                                                    {paymentProof ? (
                                                        <div className="relative w-full h-full">
                                                            <img src={paymentProof} alt="Preview" className="w-full h-full object-contain rounded-lg p-2" />
