    ...
    const handleGetLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                setIsLocating(false);
                // Feedback visual is handled by button state, but alert is good confirmation
            ...
            { enableHighAccuracy: true, timeout: 15000, maximumAge: 0 } // Request high accuracy, fresh data
        );
    };
                ...
                <div className="space-y-4 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl">
                    <div className="flex justify-between items-center mb-2">
                        <h3 className="font-bold text-lg text-white flex items-center gap-2">
                            <span className="bg-emerald-500 w-1 h-5 rounded-full inline-block"></span>
                            Entrega
                        </h3>
                        <button 
                            type="button" 
                            ...
                            disabled={isLocating}
                            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all shadow-lg transform active:scale-95 ${customer.address.googleMapsLink ? 'bg-emerald-500 text-white hover:bg-emerald-600' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                        >
                            {isLocating ? (
                                <>
