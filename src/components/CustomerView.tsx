        ...
        <div className={`fixed inset-0 z-50 flex items-end justify-center sm:items-center transition-opacity duration-300 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
            ...
            <div className={`w-full max-w-md bg-gray-900 rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[90vh] flex flex-col relative transform transition-transform duration-300 ${isClosing ? 'translate-y-full' : 'translate-y-0'}`}>
                ...
                <div className="h-64 w-full flex-shrink-0 relative">
                     ...
                     {promotion && (
                        <div className="absolute bottom-12 left-6 bg-rose-600 text-white px-3 py-1 rounded-full text-sm font-bold shadow-lg">¡Oferta Especial!</div>
                     )}
                </div>
        ...
        <div className="p-4 pb-40 animate-fade-in">
            <div className="space-y-4">
                {cartItems.map(item => {
                    ...
                    let optionsTotal = 0;
                    for (const o of options) { optionsTotal += Number(o.price || 0); }
                                ...
                                {item.selectedOptions && item.selectedOptions.length > 0 && (
                                    <div className="flex flex-wrap gap-1 mt-1 mb-1">
                                        {item.selectedOptions.map((opt, idx) => (
                                            <span key={idx} className="text-xs text-gray-400 bg-gray-700/50 px-2 py-0.5 rounded-md border border-gray-700">{opt.name}</span>
                                        ))}
                                    </div>
                                )}
...
const CheckoutView: React.FC<{
    ...
    const shippingCost = (isDelivery && settings.shipping.costType === ShippingCostType.Fixed) ? (Number(settings.shipping.fixedCost) || 0) : 0;
    ...
    return (
        <form onSubmit={handleSubmit} className="p-4 space-y-6 pb-44 animate-fade-in">
            <div className="space-y-4 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl">
                ...
                {orderType !== OrderType.DineIn && (<div><label className={labelClasses}>Número telefónico</label><input type="tel" value={customer.phone} onChange={e => setCustomer(c => ({...c, phone: e.target.value}))} className={inputClasses} placeholder="WhatsApp o celular" required /></div>)}
            </div>
            {isDelivery && (
                <div className="space-y-4 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl">
                    ...
                    <div><label className={labelClasses}>Referencias</label><input type="text" name="referencias" value={customer.address.referencias} onChange={handleAddressChange} className={inputClasses} placeholder="Color de casa, portón, etc." /></div>
                </div>
            )}
            {isPickup && (
                 <div className="p-5 bg-emerald-900/20 border border-emerald-800/50 rounded-2xl text-center">
                    <IconStore className="h-10 w-10 text-emerald-500 mx-auto mb-2"/><h3 className="font-bold text-white">Recoger en Tienda</h3>
                    <p className="text-sm text-gray-400 mt-1">{settings.branch.fullAddress}</p>
                </div>
            )}
            {settings.payment.showTipField && (
                <div className="space-y-3 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl">
                    <h3 className="font-bold text-lg text-white flex items-center gap-2"><span className="bg-emerald-500 w-1 h-5 rounded-full inline-block"></span> Propina</h3>
                    <div className="flex gap-2">
                        {[10, 15, 20].map(pct => (
                            <button key={pct} type="button" onClick={() => setTipAmount(cartTotal * (pct/100))} className={`flex-1 py-2 rounded-lg font-bold text-sm border transition-all ${Math.abs(tipAmount - (cartTotal * pct / 100)) < 0.1 ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-gray-800 border-gray-600 text-gray-300'}`}>{pct}%</button>
                        ))}
                    </div>
                </div>
            )}
            <div className="space-y-3 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl">
                 ...
                 <div className="space-y-2 pt-2">
                    {availablePaymentMethods.map(method => (
                        <div key={method}>
                            <label className={`flex justify-between items-center p-4 rounded-xl cursor-pointer transition-all border ${selectedPaymentMethod === method ? 'bg-emerald-900/20 border-emerald-500' : 'bg-gray-800 border-gray-700'}`}>
                            ...
                            {(selectedPaymentMethod === method && (method === 'Pago Móvil' || method === 'Transferencia')) && (
                                <div className="mt-2 p-4 bg-gray-700/50 rounded-xl border border-gray-600">
                                    <p className="text-xs text-gray-200 space-y-1 font-mono mb-4">
                                        {method === 'Pago Móvil' && settings.payment.pagoMovil && `Banco: ${settings.payment.pagoMovil.bank}\nTel: ${settings.payment.pagoMovil.phone}\nCI: ${settings.payment.pagoMovil.idNumber}`}
                                        {method === 'Transferencia' && settings.payment.transfer && `Banco: ${settings.payment.transfer.bank}\nCuenta: ${settings.payment.transfer.accountNumber}`}
                                    </p>
                                    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-600 rounded-xl cursor-pointer hover:bg-gray-800">
                                        {!paymentProof ? <><IconUpload className="w-8 h-8 text-gray-400"/><p className="text-xs text-gray-400">Subir comprobante</p></> : <img src={paymentProof} className="h-full w-full object-cover rounded-xl"/>}
            ...
            <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-gray-900 border-t border-gray-800 shadow-2xl p-4 safe-bottom z-30">
                <div className="flex justify-between font-bold text-xl text-white mb-4"><span>Total</span><span>${finalTotal.toFixed(2)}</span></div>
                <button type="submit" className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-4 px-6 rounded-xl shadow-lg flex items-center justify-center gap-3"><IconWhatsapp className="h-6 w-6" />Realizar Pedido</button>
            </div>
        </form>
    )
}
...
const OrderConfirmation: React.FC<{ onNewOrder: () => void, settings: AppSettings }> = ({ onNewOrder, settings }) => (
    <div className="p-6 text-center flex flex-col items-center justify-center h-[80vh] animate-fade-in">
        <div className="w-24 h-24 bg-emerald-500/20 rounded-full flex items-center justify-center mb-6"><IconCheck className="w-12 h-12 text-emerald-500"/></div>
        <h3 className="text-3xl font-bold mb-3 text-white">¡Pedido Enviado!</h3>
        <p className="text-gray-400 mb-8 text-lg">Te redirigimos a WhatsApp. El equipo de {settings.company.name} confirmará tu pedido en breve.</p>
        <button onClick={onNewOrder} className="w-full max-w-xs bg-gray-800 text-white py-4 rounded-xl font-bold">Volver al Inicio</button>
    </div>
);
...
const FooterBar: React.FC<{ itemCount: number, cartTotal: number, onViewCart: () => void }> = ({ itemCount, cartTotal, onViewCart }) => (
    <footer className="fixed bottom-4 left-4 right-4 max-w-md mx-auto z-20 animate-slide-up">
        <button onClick={onViewCart} className="w-full bg-emerald-500 text-white font-bold py-4 px-6 rounded-2xl flex justify-between items-center shadow-2xl">
            <div className="flex items-center gap-3"><div className="bg-emerald-700/50 px-3 py-1 rounded-lg text-sm font-extrabold">{itemCount}</div><span>Ver Pedido</span></div>
...
export default function CustomerView() {
    ...
    const handlePlaceOrder = async (customer: Customer, paymentMethod: PaymentMethod, tip: number = 0, proof: string | null = null) => {
        ...
        const messageParts = [
            ...
            orderType === OrderType.DineIn ? `🪑 Mesa: ${tableInfo?.table} (${tableInfo?.zone})` : `🏷️ Tipo: ${orderType}`,
        ...
        const cleanPhone = (settings.branch.whatsappNumber || '').replace(/\D/g, '');
        const url = `https://wa.me/${cleanPhone}?text=${encodeURIComponent(messageParts.filter(p => p).join('\n'))}`;
    ...
    if (isLoading || !settings) return <div className="h-screen flex items-center justify-center bg-gray-900 text-white">Sincronizando...</div>;
    ...
    return (
        <div className="bg-gray-900 min-h-screen text-gray-100">
            <div className="container mx-auto max-w-md bg-gray-900 min-h-screen shadow-2xl relative pb-24 border-x border-gray-800">
                {view !== 'menu' && <Header title={view === 'cart' ? 'Tu Pedido' : view === 'checkout' ? 'Cierre' : 'Listo'} onBack={() => setView(view === 'checkout' ? 'cart' : 'menu')} />}
