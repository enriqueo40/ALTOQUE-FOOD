            ...
            if (isFinalClosing && isTableSession) {
                ...
                if (isTableSession) {
                    msg = [
                        ...
                        (sessionItems.length > 0) ? `📈 *Total Acumulado Mesa: $${(sessionTotal + baseTotal).toFixed(2)}*` : '',
                        paymentProof ? `✅ Comprobante adjunto` : ''
                    ].filter(Boolean).join('\n');
                } else {
                    msg = [
                                ...
                                <div className="space-y-4 p-6 bg-gray-800/30 border border-gray-800 rounded-[2rem]">
                                    ...
                                    <div className="grid grid-cols-4 gap-2">
                                        {[0, 5, 10, 15].map(p => {
                                            ...
                                            return (
                                                <button key={p} type="button" onClick={() => setTipAmount(amount)} className={`py-3 rounded-xl text-xs font-bold transition-all border ${Math.abs(tipAmount - amount) < 0.1 ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-400'}`}>
                                                    {p}%
                                                </button>
                                            );
                                        })}
