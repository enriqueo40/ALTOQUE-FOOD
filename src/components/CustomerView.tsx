...
const ScheduleModal: React.FC<{ isOpen: boolean; onClose: () => void; schedules: Schedule[] }> = ({ isOpen, onClose, schedules }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-sm p-6 animate-fade-in" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={onClose} className="text-slate-500 hover:text-white"><IconX/></button>
                </div>
                <div className="space-y-3">
                    {sortedDays.map(day => (
                        <div key={day.day} className="flex justify-between items-center text-sm border-b border-slate-800 pb-3 last:border-0">
                            ...
                            <div className="text-right">
                                {!day.isOpen || day.shifts.length === 0 ? (
                                    ...
                                    day.shifts.map((shift, index) => (
                                        <span key={index} className="block text-slate-400 font-mono">{shift.start} - {shift.end}</span>
                                    ))
                                )}
                            </div>
                        </div>
                    ...
                    {view === 'menu' && (
                        <div className="animate-fade-in">
                            ...
                            <div className="pt-8 pb-6 flex flex-col items-center text-center -mt-16 relative z-10">
                                <div className="w-24 h-24 bg-slate-800 rounded-full p-1 shadow-2xl mb-4 border-4 border-slate-900 overflow-hidden">
                                    {settings.branch.logoUrl ? 
                                        ...
                                        : <div className="w-full h-full flex items-center justify-center font-black text-emerald-400 text-3xl bg-slate-700">{settings.company.name.slice(0,2)}</div>
                                    }
                                </div>

                                <h1 className="text-2xl font-bold text-white mt-0 tracking-tight">{settings.company.name}</h1>
                                <p className="text-sm text-slate-400 mt-1">{settings.branch.alias}</p>

                                <div className="mt-4 space-y-2 text-xs">
                                    <div className="flex items-center justify-center gap-2">
                                        <span className={`w-2 h-2 rounded-full ${storeStatus.isOpen ? 'bg-green-400 animate-pulse' : 'bg-slate-500'}`}></span>
                                        <span className={`font-bold ${storeStatus.isOpen ? 'text-green-400' : 'text-slate-500'}`}>{storeStatus.message}</span>
                                        <button onClick={() => setIsScheduleModalOpen(true)} className="text-slate-400 underline hover:text-white transition-colors">Ver horarios</button>
                                    </div>
                                    
                                    {settings.branch.googleMapsLink && settings.branch.fullAddress && (
                                        <a href={settings.branch.googleMapsLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-1.5 text-slate-500 hover:text-slate-300 transition-colors pt-1">
