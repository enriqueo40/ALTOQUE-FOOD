...
import { IconPlus, IconMinus, IconArrowLeft, IconTrash, IconX, IconWhatsapp, IconSearch, IconLocationMarker, IconStore, IconCheck, IconSparkles, IconInfo, IconUpload } from '../constants';
...
export default function CustomerView() {
    ...
    const handlePlaceOrder = async () => {
        ...
        const customer: Customer = { name: customerName, phone: customerPhone, address: customerAddress };
        const newOrder: Omit<Order, 'id' | 'createdAt'> = {
            customer, items: cartItems, total: finalTotal, status: OrderStatus.Pending, branchId: 'main',
            generalComments, orderType, tableId: tableInfo ? `${tableInfo.zone} - ${tableInfo.table}` : undefined,
            paymentStatus: 'pending', paymentProof: paymentProof || undefined
        };
    ...
    if (view === 'checkout') {
        ...
        return (
            <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col">
                <header className="p-4 bg-white dark:bg-gray-800 flex items-center gap-3 border-b dark:border-gray-700"><button onClick={() => setView('cart')}><IconArrowLeft/></button><h1 className="font-bold dark:text-white">Finalizar Pedido</h1></header>
                <div className="flex-1 p-4 space-y-6 overflow-y-auto">
                    <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm space-y-4">
                        <h3 className="font-bold dark:text-white">Tipo de entrega</h3>
                        <div className="grid grid-cols-2 gap-3">
                            <button onClick={() => setOrderType(OrderType.Delivery)} className={`p-3 rounded-xl border flex flex-col items-center gap-2 ${orderType === OrderType.Delivery ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700' : 'dark:border-gray-700 dark:text-gray-400'}`}><IconStore/> Domicilio</button>
                            <button onClick={() => setOrderType(OrderType.TakeAway)} className={`p-3 rounded-xl border flex flex-col items-center gap-2 ${orderType === OrderType.TakeAway ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700' : 'dark:border-gray-700 dark:text-gray-400'}`}><IconLocationMarker/> Para llevar</button>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm space-y-4">
                        <h3 className="font-bold dark:text-white">Tus Datos</h3>
                        <input type="text" placeholder="Tu Nombre" value={customerName} onChange={e => setCustomerName(e.target.value)} className="w-full p-3 bg-gray-50 dark:bg-gray-700 rounded-xl outline-none dark:text-white" />
                        <input type="tel" placeholder="Tu Teléfono" value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} className="w-full p-3 bg-gray-50 dark:bg-gray-700 rounded-xl outline-none dark:text-white" />
                        {orderType === OrderType.Delivery && <input type="text" placeholder="Dirección Completa" value={customerAddress.calle} onChange={e => setCustomerAddress({...customerAddress, calle: e.target.value})} className="w-full p-3 bg-gray-50 dark:bg-gray-700 rounded-xl outline-none dark:text-white" />}
                    </div>
                    <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm space-y-4">
                        <h3 className="font-bold dark:text-white">Método de Pago</h3>
                        <div className="grid grid-cols-2 gap-2">
                            {settings?.payment[orderType === OrderType.Delivery ? 'deliveryMethods' : 'pickupMethods'].map(m => (
                                <button key={m} onClick={() => { setSelectedPaymentMethod(m); setPaymentProof(null); }} className={`p-3 rounded-lg border text-sm font-bold ${selectedPaymentMethod === m ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-gray-50 dark:bg-gray-700 dark:border-gray-700 dark:text-gray-300'}`}>{m}</button>
                            ))}
                        </div>
                        {/* SECCIÓN MEJORADA DE DATOS DE PAGO */}
                        {selectedPaymentMethod && selectedPaymentMethod !== 'Efectivo' && selectedPaymentMethod !== 'Punto de Venta' && (
                            <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-900 border dark:border-gray-700 rounded-2xl animate-in fade-in slide-in-from-top-2 duration-300">
                                {selectedPaymentMethod === 'Pago Móvil' && (
                                    <div className="space-y-2 mb-4">
                                        <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 font-bold text-xs uppercase tracking-widest"><IconInfo className="h-4 w-4"/> Datos para Pago Móvil</div>
                                        <div className="grid grid-cols-1 gap-1 text-sm bg-white dark:bg-gray-800 p-3 rounded-xl shadow-inner">
                                            <div className="flex justify-between"><span className="text-gray-500">Banco:</span> <span className="font-bold dark:text-white">{settings.payment.pagoMovil?.bank || '---'}</span></div>
                                            <div className="flex justify-between"><span className="text-gray-500">Teléfono:</span> <span className="font-mono font-bold dark:text-white">{settings.payment.pagoMovil?.phone || '---'}</span></div>
                                            <div className="flex justify-between"><span className="text-gray-500">Cédula/RIF:</span> <span className="font-mono font-bold dark:text-white">{settings.payment.pagoMovil?.idNumber || '---'}</span></div>
                                        </div>
                                    </div>
                                )}
                                {selectedPaymentMethod === 'Transferencia' && (
                                    <div className="space-y-2 mb-4">
                                        <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 font-bold text-xs uppercase tracking-widest"><IconInfo className="h-4 w-4"/> Datos para Transferencia</div>
                                        <div className="grid grid-cols-1 gap-1 text-sm bg-white dark:bg-gray-800 p-3 rounded-xl shadow-inner">
                                            <div className="flex justify-between"><span className="text-gray-500">Banco:</span> <span className="font-bold dark:text-white">{settings.payment.transfer?.bank || '---'}</span></div>
                                            <div className="flex flex-col gap-1 border-y dark:border-gray-700 py-2 my-1"><span className="text-gray-500 text-xs uppercase">Cuenta:</span> <span className="font-mono font-bold dark:text-white break-all text-xs">{settings.payment.transfer?.accountNumber || '---'}</span></div>
                                            <div className="flex justify-between"><span className="text-gray-500">Titular:</span> <span className="font-bold dark:text-white">{settings.payment.transfer?.accountHolder || '---'}</span></div>
                                            <div className="flex justify-between"><span className="text-gray-500">RIF:</span> <span className="font-mono font-bold dark:text-white">{settings.payment.transfer?.idNumber || '---'}</span></div>
                                        </div>
                                    </div>
                                )}
                                <div className="space-y-3">
                                    <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">Sube tu comprobante</p>
                                    {!paymentProof ? (
                                        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-2xl bg-white dark:bg-gray-800 cursor-pointer hover:border-emerald-500 transition-colors group">
                                            <IconUpload className="h-8 w-8 text-gray-400 group-hover:text-emerald-500 mb-2"/>
                                            <span className="text-sm text-gray-500 font-medium">Adjuntar captura</span>
                                            <input type="file" className="hidden" accept="image/*" onChange={handleProofUpload} />
                                        </label>
                                    ) : (
                                        <div className="relative rounded-2xl overflow-hidden shadow-lg border-2 border-emerald-500">
                                            <img src={paymentProof} className="w-full h-40 object-cover" />
                                            <button onClick={() => setPaymentProof(null)} className="absolute top-2 right-2 bg-red-500 text-white p-1.5 rounded-full shadow-lg"><IconX className="h-4 w-4"/></button>
                                            <div className="absolute bottom-0 left-0 right-0 bg-emerald-600 text-white text-[10px] font-bold text-center py-1 uppercase tracking-tighter">Imagen Cargada Correctamente</div>
                                        </div>
                                    )}
                                </div>
                            </div>
                ...
                <div className="p-4 bg-white dark:bg-gray-800 border-t dark:border-gray-700 shadow-2xl space-y-4">
                    <div className="flex justify-between items-center text-xl font-bold dark:text-white"><span>Total:</span> <span className="text-emerald-600">${(cartTotal + shippingCost).toFixed(2)}</span></div>
                    <button onClick={handlePlaceOrder} disabled={isPlacingOrder} className="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold shadow-lg flex items-center justify-center gap-3 active:scale-95 transition-all disabled:opacity-50">
                        {isPlacingOrder ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <><IconWhatsapp className="h-6 w-6"/> Confirmar por WhatsApp</>}
                    </button>
                </div>
            </div>
        );
