...
// ... (ScheduleModal and RestaurantHero remain largely same, just small rendering tweaks if needed) ...
...
const RestaurantHero: React.FC<{ 
    ...
    const getTimeLabel = () => orderType === OrderType.TakeAway ? "Tiempo recogida" : "Tiempo envío";

    const getTimeText = () => {
        if (orderType === OrderType.TakeAway) return `${shipping.pickupTime.min} min`;
        ...
        <div className="relative">
            <div className="h-48 w-full overflow-hidden relative">
                {branch.coverImageUrl ? (
                    <img src={branch.coverImageUrl} alt="Cover" className="w-full h-full object-cover transition-opacity duration-500 opacity-0" onLoad={(e) => (e.target as HTMLImageElement).classList.remove('opacity-0')} fetchPriority="high" />
                ) : (
            ...
            <div className="px-6 relative -mt-16 flex flex-col items-center text-center pb-6 border-b border-gray-800">
                ...
                <div className="flex flex-col items-center gap-1 mb-4">
                    <p className="text-sm text-gray-400 flex items-center gap-1">{branch.alias}</p>
                    {currentSchedule && (
            ...
            {currentSchedule && (
                <ScheduleModal isOpen={showSchedule} onClose={() => setShowSchedule(false)} schedule={currentSchedule} />
            )}
        </div>
    );
};
...
// Memoized Product Row for better scroll performance
...
}, (prev, next) => prev.quantityInCart === next.quantityInCart && prev.product.id === next.product.id && prev.product.available === next.product.available);
...
const MenuList: React.FC<{
    ...
    if (isLoading) {
        return (
            <div className="px-4 mt-20 space-y-8 animate-pulse">
                {[1, 2].map(i => (
                    <div key={i}>
                        <Skeleton className="h-8 w-48 mb-4 bg-gray-800" />
                        <div className="space-y-4">
                            {[1, 2, 3].map(j => (
                                <div key={j} className="h-32 bg-gray-800 rounded-xl" />
                            ))}
                        </div>
                    </div>
                ))}
    ...
    return (
        <div className="pb-24">
            <div className="sticky top-0 z-10 bg-gray-900/95 backdrop-blur shadow-lg pt-2 transition-all">
...
// ... (ProductDetailModal, CartSummaryView, CheckoutView, OrderConfirmation, FooterBar remain the same, logic is fine) ...
// Including them fully would make this response huge, assuming they are unchanged except for imports. 
// I will include the full updated CustomerView logic flow below with the optimizations applied.
...
const ProductDetailModal: React.FC<{
    ...
    // ... same content as previous, standard React logic ...
    ...
    return (
        <div className={`fixed inset-0 z-50 flex items-end justify-center sm:items-center transition-opacity duration-300 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
            ...
            <div className={`w-full max-w-md bg-gray-900 rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[90vh] flex flex-col relative transform transition-transform duration-300 ${isClosing ? 'translate-y-full' : 'translate-y-0'}`}>
                 <button onClick={handleClose} className="absolute top-4 right-4 bg-black/40 hover:bg-black/60 p-2 rounded-full text-white z-10 backdrop-blur-md transition-colors"><IconX className="h-6 w-6" /></button>
                <div className="h-64 w-full flex-shrink-0 relative">
                     ...
                     {promotion && <div className="absolute bottom-12 left-6 bg-rose-600 text-white px-3 py-1 rounded-full text-sm font-bold shadow-lg">¡Oferta Especial!</div>}
                </div>
                <div className="p-6 flex-grow overflow-y-auto -mt-12 relative z-10">
                            ...
                            <div key={p.id} className="bg-gray-800/50 p-4 rounded-xl border border-gray-700">
                                ...
                                <div className="space-y-2">
                                    {p.options.filter(o => o.available).map(opt => {
                                        ...
                                        return (
                                            <div key={opt.id} onClick={() => handleOptionToggle(p, opt)} className={`flex justify-between items-center p-3 rounded-lg cursor-pointer border transition-all ${isSelected ? 'bg-emerald-500/20 border-emerald-500 ring-1 ring-emerald-500 shadow-lg shadow-emerald-900/20' : 'bg-gray-800 border-gray-700 hover:border-gray-600'}`}>
                    ...
                    <div className="mt-6">
                        ...
                        <textarea value={comments} onChange={(e) => setComments(e.target.value)} rows={3} className="w-full p-3 rounded-xl bg-gray-800 border border-gray-700 text-white placeholder-gray-500 focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all resize-none" placeholder="Ej. Sin cebolla, salsa aparte..."/>
                    </div>
                </div>
                <div className="p-4 border-t border-gray-800 bg-gray-900 rounded-b-3xl sm:rounded-b-2xl safe-bottom">
                    ...
                    <button onClick={handleAdd} className="w-full font-bold py-4 px-6 rounded-xl transition-all transform active:scale-[0.98] shadow-lg flex justify-between items-center bg-emerald-500 hover:bg-emerald-600 text-white shadow-emerald-900/20">
...
const CartSummaryView: React.FC<{ cartItems: CartItem[], cartTotal: number, onUpdateQuantity: (id: string, q: number) => void, onRemoveItem: (id: string) => void, generalComments: string, onGeneralCommentsChange: (c: string) => void, onProceedToCheckout: () => void }> = ({ cartItems, cartTotal, onUpdateQuantity, onRemoveItem, generalComments, onGeneralCommentsChange, onProceedToCheckout }) => {
    if (cartItems.length === 0) {
        return (
            <div className="p-8 text-center h-[calc(100vh-80px)] flex flex-col items-center justify-center text-gray-400">
                 <div className="w-24 h-24 bg-gray-800 rounded-full flex items-center justify-center mb-6"><IconClock className="h-10 w-10 opacity-50" /></div>
    ...
    return (
        <div className="p-4 pb-40 animate-fade-in">
            <div className="space-y-4">
                {cartItems.map(item => {
                    const optionsTotal = item.selectedOptions ? item.selectedOptions.reduce((acc, curr) => acc + (Number(curr.price) || 0), 0) : 0;
                    ...
                    return (
                        <div key={item.cartItemId} className="flex gap-4 bg-gray-800/50 p-3 rounded-xl border border-gray-800">
                            ...
                            <div className="flex-grow flex flex-col justify-between">
                                ...
                                {item.selectedOptions && item.selectedOptions.length > 0 && <div className="flex flex-wrap gap-1 mt-1 mb-1">{item.selectedOptions.map((opt, idx) => (<span key={idx} className="text-xs text-gray-400 bg-gray-700/50 px-2 py-0.5 rounded-md border border-gray-700">{opt.name}</span>))}</div>}
             ...
             <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-gray-900 border-t border-gray-800 shadow-2xl p-4 safe-bottom z-30">
                <div className="flex justify-between items-center mb-4 px-2">
                 ...
                 <button onClick={onProceedToCheckout} className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-4 px-6 rounded-xl shadow-lg shadow-emerald-900/20 transition-all transform active:scale-[0.98]">Continuar</button>
            </div>
        </div>
    )
}

// ... CheckoutView and OrderConfirmation remain as in previous files, imports handled ...
const CheckoutView: React.FC<{ cartTotal: number, onPlaceOrder: (customer: Customer, paymentMethod: PaymentMethod, tipAmount: number, paymentProof?: string | null) => void, settings: AppSettings, orderType: OrderType }> = ({ cartTotal, onPlaceOrder, settings, orderType }) => {
    ...
    const handleAddressChange = (e: React.ChangeEvent<HTMLInputElement>) => { const { name, value } = e.target; setCustomer(prev => ({ ...prev, address: { ...prev.address, [name]: value } })); };
    const handleTipSelection = (percentage: number) => { setTipAmount(cartTotal * (percentage / 100)); };
    const handleProofUpload = (e: React.ChangeEvent<HTMLInputElement>) => { if (e.target.files && e.target.files[0]) { const file = e.target.files[0]; const reader = new FileReader(); reader.onloadend = () => { setPaymentProof(reader.result as string); }; reader.readAsDataURL(file); } };
    const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onPlaceOrder(customer, selectedPaymentMethod, tipAmount, paymentProof); };
    ...
    return (
        <form onSubmit={handleSubmit} className="p-4 space-y-6 pb-44 animate-fade-in">
            <div className="space-y-4 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl">
                ...
                <div><label className={labelClasses}>Nombre</label><input type="text" value={customer.name} onChange={e => setCustomer(c => ({...c, name: e.target.value}))} className={inputClasses} placeholder="Tu nombre completo" required /></div>
                 {!isDineIn && (<div><label className={labelClasses}>Número telefónico</label><input type="tel" value={customer.phone} onChange={e => setCustomer(c => ({...c, phone: e.target.value}))} className={inputClasses} placeholder="WhatsApp o celular" required /></div>)}
            </div>
            {isDelivery && (
                <div className="space-y-4 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div><label className={labelClasses}>Número Ext.</label><input type="text" name="numero" value={customer.address.numero} onChange={handleAddressChange} className={inputClasses} required /></div>
                         <div><label className={labelClasses}>Colonia</label><input type="text" name="colonia" value={customer.address.colonia} onChange={handleAddressChange} className={inputClasses} required /></div>
                    </div>
                    <div><label className={labelClasses}>Referencias <span className="font-normal text-gray-500 text-xs">(Opcional)</span></label><input type="text" name="referencias" value={customer.address.referencias} onChange={handleAddressChange} className={inputClasses} placeholder="Color de casa, portón, etc." /></div>
                </div>
            )}
            {isPickup && ( <div className="p-5 bg-emerald-900/20 border border-emerald-800/50 rounded-2xl text-center"><IconStore className="h-10 w-10 text-emerald-500 mx-auto mb-2"/><h3 className="font-bold text-white">Recoger en Tienda</h3><p className="text-sm text-gray-400 mt-1">No olvides pasar a recoger tu pedido.</p><p className="text-sm text-gray-400 mt-1 font-medium">{settings.branch.fullAddress}</p></div>)}
            {settings.payment.showTipField && (
                <div className="space-y-3 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl">
                    ...
                    <div className="flex gap-2">
                        {[10, 15, 20].map(pct => ( <button key={pct} type="button" onClick={() => handleTipSelection(pct)} className={`flex-1 py-2 rounded-lg font-bold text-sm border transition-all ${Math.abs(tipAmount - (cartTotal * pct / 100)) < 0.1 ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-gray-800 border-gray-600 text-gray-300 hover:border-gray-500'}`}>{pct}%</button>))}
                        <div className="flex-1 relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">$</span><input type="number" placeholder="Otro" value={tipAmount || ''} onChange={(e) => setTipAmount(parseFloat(e.target.value) || 0)} className="w-full py-2 pl-6 pr-2 bg-gray-800 border border-gray-600 rounded-lg text-white text-sm focus:border-emerald-500 outline-none"/></div>
                    </div>
                </div>
            )}
            <div className="space-y-3 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl">
                        ...
                        <div key={method}>
                            ...
                            {(selectedPaymentMethod === method && (method === 'Pago Móvil' || method === 'Transferencia')) && (
                                <div className="mt-2 p-4 bg-gray-700/50 rounded-xl border border-gray-600 animate-fade-in">
                                    <div className="flex justify-between items-start mb-2"><h4 className="text-emerald-400 font-bold text-sm uppercase">Datos {method}</h4><button type="button" onClick={() => {let text = ''; if (method === 'Pago Móvil' && settings.payment.pagoMovil) {text = `Banco: ${settings.payment.pagoMovil.bank}\nTel: ${settings.payment.pagoMovil.phone}\nCI/RIF: ${settings.payment.pagoMovil.idNumber}`;} else if (method === 'Transferencia' && settings.payment.transfer) {text = `Banco: ${settings.payment.transfer.bank}\nCuenta: ${settings.payment.transfer.accountNumber}\nTitular: ${settings.payment.transfer.accountHolder}\nCI/RIF: ${settings.payment.transfer.idNumber}`;} navigator.clipboard.writeText(text); alert('Datos copiados');}} className="text-xs text-gray-400 hover:text-white flex items-center gap-1"><IconDuplicate className="h-3 w-3"/> Copiar</button></div>
                                    <div className="text-sm text-gray-200 space-y-1 font-mono mb-4">
                                        {method === 'Pago Móvil' && settings.payment.pagoMovil && (<><p><span className="text-gray-500">Banco:</span> {settings.payment.pagoMovil.bank}</p><p><span className="text-gray-500">Teléfono:</span> {settings.payment.pagoMovil.phone}</p><p><span className="text-gray-500">Cédula/RIF:</span> {settings.payment.pagoMovil.idNumber}</p></>)}
                                        {method === 'Transferencia' && settings.payment.transfer && (<><p><span className="text-gray-500">Banco:</span> {settings.payment.transfer.bank}</p><p><span className="text-gray-500">Cuenta:</span> {settings.payment.transfer.accountNumber}</p><p><span className="text-gray-500">Titular:</span> {settings.payment.transfer.accountHolder}</p><p><span className="text-gray-500">CI/RIF:</span> {settings.payment.transfer.idNumber}</p></>)}
                                    </div>
                                    <div className="mt-4 border-t border-gray-700/50 pt-4"><p className="text-xs font-bold text-gray-400 uppercase mb-2">Comprobante de pago</p>{!paymentProof ? (<label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-600 rounded-xl cursor-pointer hover:bg-gray-800 transition-colors bg-gray-800/30"><div className="flex flex-col items-center justify-center pt-5 pb-6"><IconUpload className="w-8 h-8 text-gray-400 mb-2" /><p className="text-xs text-gray-400 font-medium">Toca para subir captura</p></div><input type="file" className="hidden" accept="image/*" onChange={handleProofUpload} /></label>) : (<div className="relative w-full h-48 rounded-xl overflow-hidden border border-emerald-500/50 shadow-lg"><img src={paymentProof} alt="Comprobante" className="w-full h-full object-cover" /><div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity"><button type="button" onClick={() => setPaymentProof(null)} className="bg-red-500 text-white p-3 rounded-full shadow-lg transform hover:scale-110 transition-transform"><IconTrash className="h-6 w-6" /></button></div><div className="absolute bottom-2 right-2 bg-emerald-500 text-white text-[10px] px-3 py-1 rounded-full font-bold shadow-lg flex items-center gap-1"><IconCheck className="h-3 w-3" /> Cargado</div></div>)}</div>
                                </div>
                            )}
                        </div>
                    ))}
            ...
            <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-gray-900 border-t border-gray-800 shadow-2xl p-4 safe-bottom z-30">
                <div className="space-y-2 mb-4 text-sm px-2"><div className="flex justify-between text-gray-400"><span>Subtotal</span><span>${cartTotal.toFixed(2)}</span></div>{isDelivery && (<div className="flex justify-between text-gray-400"><span>Envío</span><span>{shippingCost > 0 ? `$${shippingCost.toFixed(2)}` : "Por cotizar"}</span></div>)}{tipAmount > 0 && (<div className="flex justify-between text-emerald-400"><span>Propina</span><span>${tipAmount.toFixed(2)}</span></div>)}<div className="flex justify-between font-bold text-xl text-white pt-2 border-t border-gray-800"><span>Total</span><span>${finalTotal.toFixed(2)} {isDelivery && shippingCost === 0 && "+ envío"}</span></div></div>
                 <button type="submit" className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-4 px-6 rounded-xl shadow-lg flex items-center justify-center gap-3 transition-all transform active:scale-[0.98]"><IconWhatsapp className="h-6 w-6" />Confirmar Pedido</button>
            </div>
        </form>
    )
}
...
const OrderConfirmation: React.FC<{ onNewOrder: () => void, settings: AppSettings, whatsappUrl: string | null }> = ({ onNewOrder, settings, whatsappUrl }) => (
    <div className="p-6 text-center flex flex-col items-center justify-center h-[80vh] animate-fade-in">
        ...
        <p className="text-gray-400 mb-8 text-lg leading-relaxed">Tu pedido ha sido registrado en el sistema. Para finalizar, envíalo a nuestro WhatsApp.</p>
        {whatsappUrl && (<a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="w-full max-w-xs bg-green-600 hover:bg-green-500 text-white py-4 rounded-xl font-bold transition-all shadow-lg flex items-center justify-center gap-2 mb-4 animate-bounce"><IconWhatsapp className="h-6 w-6"/> Enviar a WhatsApp</a>)}
...
const FooterBar: React.FC<{ itemCount: number, cartTotal: number, onViewCart: () => void }> = ({ itemCount, cartTotal, onViewCart }) => {
    return (<footer className="fixed bottom-4 left-4 right-4 max-w-md mx-auto z-20 animate-slide-up"><button onClick={onViewCart} className="w-full bg-emerald-500 text-white font-bold py-4 px-6 rounded-2xl flex justify-between items-center shadow-2xl shadow-emerald-900/50 hover:bg-emerald-400 transition-transform hover:-translate-y-1 active:translate-y-0"><div className="flex items-center gap-3"><div className="bg-emerald-700/50 px-3 py-1 rounded-lg text-sm font-extrabold border border-emerald-400/30">{itemCount}</div><span>Ver Pedido</span></div><span className="text-lg">${cartTotal.toFixed(2)}</span></button></footer>);
};
...
export default function CustomerView() {
    ...
    const handleAddToCart = (product: Product, quantity: number, comments?: string, options: PersonalizationOption[] = []) => { addToCart(product, quantity, comments, options); setSelectedProduct(null); };
    const handleStartNewOrder = () => { clearCart(); setGeneralComments(''); setView('menu'); };
    ...
    const handlePlaceOrder = async (customer: Customer, paymentMethod: PaymentMethod, tipAmount: number = 0, paymentProof: string | null = null) => {
        ...
        try { await saveOrder(newOrder); } catch(e) { alert("Hubo un error al guardar tu pedido. Por favor, intenta de nuevo."); return; }
        
        // WhatsApp Logic
        const formatOptions = (item: CartItem) => { if (!item.selectedOptions || item.selectedOptions.length === 0) return ''; return item.selectedOptions.map(opt => `   + ${opt.name}`).join('\n'); };
        const itemDetails = cartItems.map(item => { let detail = `▪️ ${item.quantity}x ${item.name}`; const optionsStr = formatOptions(item); if (optionsStr) detail += `\n${optionsStr}`; if (item.comments) detail += `\n   _Nota: ${item.comments}_`; return detail; });
    ...
    const getHeaderTitle = () => { switch(view) { case 'cart': return 'Tu pedido'; case 'checkout': return 'Completa tu pedido'; case 'confirmation': return 'Confirmación'; default: return ''; } };
    const canGoBack = view === 'cart' || view === 'checkout';
    const handleBack = () => { if (view === 'checkout') setView('cart'); else if (view === 'cart') setView('menu'); }
            ...
            <div className="container mx-auto max-w-md bg-gray-900 min-h-screen shadow-2xl relative pb-24 border-x border-gray-800">
                ...
                {view === 'menu' && settings && (
                    <>
                        ...
                        <MenuList products={allProducts} categories={allCategories} onProductClick={handleProductClick} cartItems={cartItems} currency={settings.company.currency.code} promotions={allPromotions} isLoading={isLoading} />
                    </>
                )}
                ...
                {view === 'checkout' && settings && <CheckoutView cartTotal={cartTotal} onPlaceOrder={handlePlaceOrder} settings={settings} orderType={orderType} />}
                {view === 'confirmation' && settings && <OrderConfirmation onNewOrder={handleStartNewOrder} settings={settings} whatsappUrl={lastWhatsappUrl} />}
                {selectedProduct && <ProductDetailModal product={selectedProduct} onAddToCart={handleAddToCart} onClose={() => setSelectedProduct(null)} personalizations={allPersonalizations} promotions={allPromotions} />}
