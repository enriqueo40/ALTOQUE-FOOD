...
const RestaurantHero: React.FC<{ 
    ...
    const getTimeText = () => orderType === OrderType.TakeAway ? `${shipping.pickupTime.min} min` : `${shipping.deliveryTime.min} - ${shipping.deliveryTime.max} min`;
...
const ProductRow: React.FC<{ product: Product; quantityInCart: number; onClick: () => void; currency: string; promotions: Promotion[] }> = ({ product, quantityInCart, onClick, currency, promotions }) => {
    ...
    return (
        <div onClick={onClick} className="bg-gray-800 rounded-xl p-3 flex gap-4 hover:bg-gray-750 active:scale-[0.99] transition-all cursor-pointer border border-gray-700 shadow-sm group relative overflow-hidden h-32">
            {promotion && <div className="absolute top-0 left-0 bg-rose-500 text-white text-[10px] font-bold px-2 py-1 rounded-br-lg z-10">-{promotion.discountType === DiscountType.Percentage ? `${promotion.discountValue}%` : `$${promotion.discountValue}`}</div>}
            <div className="relative h-24 w-24 flex-shrink-0 self-center">
                ...
                {quantityInCart > 0 && <div className="absolute -top-2 -right-2 bg-emerald-500 text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center shadow-lg ring-2 ring-gray-900">{quantityInCart}</div>}
            </div>
            <div className="flex-1 flex flex-col justify-between py-1">
                ...
                <div className="flex justify-between items-end mt-2">
                    ...
                    <div className="bg-gray-700 p-1.5 rounded-full text-emerald-400 group-hover:bg-emerald-500 group-hover:text-white transition-colors"><IconPlus className="h-4 w-4" /></div>
                </div>
            </div>
        </div>
    );
...
const MenuList: React.FC<{ products: Product[], categories: Category[], onProductClick: (product: Product) => void, cartItems: CartItem[], currency: string, promotions: Promotion[], isLoading: boolean }> = ({ products, categories, onProductClick, cartItems, currency, promotions, isLoading }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [activeCategory, setActiveCategory] = useState<string>('');
    useEffect(() => { if (categories.length > 0 && !activeCategory) setActiveCategory(categories[0].id); }, [categories, activeCategory]);
    ...
    const filteredProducts = useMemo(() => products.filter(p => p.available && p.name.toLowerCase().includes(searchTerm.toLowerCase())), [products, searchTerm]);
    const groupedProducts = useMemo(() => {
        const standardGroups = categories.map(category => ({ ...category, products: filteredProducts.filter(p => p.categoryId === category.id) })).filter(category => category.products.length > 0);
                ...
                {groupedProducts.map(category => (
                    <div key={category.id} id={`category-${category.id}`} className="scroll-mt-40">
                        ...
                        <div className="grid gap-4">
                            {category.products.map(product => <ProductRow key={product.id} product={product} quantityInCart={productQuantities[product.id] || 0} onClick={() => onProductClick(product)} currency={currency} promotions={promotions} />)}
                        </div>
                    </div>
                ))}
            </div>
...
const ProductDetailModal: React.FC<{ product: Product, onAddToCart: (product: Product, quantity: number, comments?: string, options?: PersonalizationOption[]) => void, onClose: () => void, personalizations: Personalization[], promotions: Promotion[] }> = ({product, onAddToCart, onClose, personalizations, promotions}) => {
    ...
    Object.values(selectedOptions).forEach((group: PersonalizationOption[]) => group.forEach(opt => { totalOptionsPrice += Number(opt.price || 0); }));
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
            ...
            <div className="w-full max-w-md bg-gray-900 rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[90vh] flex flex-col relative animate-slide-up">
                ...
                <div className="p-6 flex-grow overflow-y-auto -mt-12 relative z-10 text-white">
                    ...
                    <div className="mt-6"><textarea value={comments} onChange={(e) => setComments(e.target.value)} rows={3} className="w-full p-3 rounded-xl bg-gray-800 border border-gray-700 text-white placeholder-gray-500" placeholder="Instrucciones especiales..." /></div>
                </div>
                <div className="p-4 border-t border-gray-800 bg-gray-900 rounded-b-3xl sm:rounded-b-2xl">
                    ...
                    <button onClick={() => onAddToCart({ ...product, price: basePrice }, quantity, comments, (Object.values(selectedOptions) as PersonalizationOption[][]).reduce((acc, curr) => acc.concat(curr), []))} className="w-full font-bold py-4 px-6 rounded-xl bg-emerald-500 text-white flex justify-between items-center shadow-lg"><span>Agregar al pedido</span><span>${totalPrice.toFixed(2)}</span></button>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <form onSubmit={(e) => { e.preventDefault(); onPlaceOrder(customer, selectedPaymentMethod, tipAmount, paymentProof); }} className="p-4 space-y-6 pb-44 animate-fade-in">
            <div className="space-y-4 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl text-white">
                <h3 className="font-bold text-lg flex items-center gap-2"><span className="bg-emerald-500 w-1 h-5 rounded-full"></span> Tus datos</h3>
                <input type="text" value={customer.name} onChange={e => setCustomer(c => ({...c, name: e.target.value}))} className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Nombre completo" required />
                {orderType !== OrderType.DineIn && <input type="tel" value={customer.phone} onChange={e => setCustomer(c => ({...c, phone: e.target.value}))} className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500" placeholder="WhatsApp" required />}
            </div>
            ...
            {isDelivery && (
                <div className="space-y-4 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl text-white">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <input type="text" name="colonia" value={customer.address.colonia} onChange={handleAddressChange} className="px-4 py-3 bg-gray-800 border border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Colonia" required />
                    </div>
                    <input type="text" name="referencias" value={customer.address.referencias} onChange={handleAddressChange} className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Referencias (color casa, portón, etc)" />
                </div>
            )}

            <div className="space-y-3 p-5 bg-gray-800/30 border border-gray-800 rounded-2xl text-white">
                 <h3 className="font-bold text-lg flex items-center gap-2"><span className="bg-emerald-500 w-1 h-5 rounded-full"></span> Pago</h3>
                 <div className="space-y-2">
...
const OrderConfirmation: React.FC<{ onNewOrder: () => void, settings: AppSettings }> = ({ onNewOrder, settings }) => (
    <div className="p-6 text-center flex flex-col items-center justify-center h-[80vh] animate-fade-in text-white">
        ...
        <h3 className="text-3xl font-bold mb-3">¡Pedido Enviado!</h3>
        <p className="text-gray-400 mb-8 text-lg">Te redirigimos a WhatsApp para confirmar los detalles.</p>
