...
const PairingAI: React.FC<{ items: CartItem[], allProducts: Product[] }> = ({ items, allProducts }) => {
    ...
    return (
        <div className="mx-4 mb-4 p-4 bg-indigo-900/20 border border-indigo-500/20 rounded-2xl flex items-start gap-3 animate-fade-in">
            <div className="p-2 bg-indigo-500/20 rounded-lg text-indigo-400">
                <IconSparkles className="h-5 w-5" />
            </div>
            <div className="flex-1">
                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Sugerencia del Chef (IA)</p>
                {loading ? (
...
// Fix: Added missing ProductDetailModal component
...
export default function CustomerView() {
    ...
    const handleOrder = async (customer: Customer, payment: PaymentMethod, proof?: string | null) => {
        ...
        } catch(e) { alert("Error al procesar. Reintenta."); }
    };

    if (!settings) return <div className="h-screen bg-gray-900 flex items-center justify-center text-emerald-500 font-bold animate-pulse">Cargando ANYVAL PARK...</div>;
    ...
    return (
        <div className="bg-gray-900 min-h-screen text-gray-100 font-sans selection:bg-emerald-500/30">
            <div className="container mx-auto max-w-md bg-gray-900 min-h-screen relative shadow-2xl border-x border-gray-800">
                ...
                {view !== 'menu' && (
                    <Header 
                        title={view === 'cart' ? 'MI PEDIDO' : (isFinalPayment ? 'PAGAR CUENTA' : 'CONFIRMAR')} 
                ...
                {view === 'confirmation' && (
                    <div className="p-10 text-center h-[85vh] flex flex-col items-center justify-center gap-6 animate-fade-in">
                        ...
                        <h2 className="text-3xl font-black text-white">{isFinalPayment ? '¡CUENTA CERRADA!' : '¡PEDIDO EN COCINA!'}</h2>
                        <p className="text-gray-400 text-sm leading-relaxed max-w-xs">
                            {isFinalPayment ? 'Gracias por visitarnos. El personal validará tu comprobante en segundos.' : 'Tu pedido ya fue notificado. Puedes seguir pidiendo más cosas desde el menú.'}
                        </p>
                    ...
                    <div className="fixed bottom-8 left-6 right-6 max-w-md mx-auto z-40 flex flex-col gap-3">
                        {activeOrderId && (
                            <button 
                                ...
                                className="w-full bg-gray-800/95 backdrop-blur-md text-white font-black py-3.5 px-6 rounded-2xl flex justify-between items-center border border-emerald-500/40 shadow-2xl"
                            >
                                ...
                                <span className="bg-emerald-500 px-3 py-1 rounded-lg text-xs">${sessionTotal.toFixed(2)}</span>
                            </button>
                        )}
                        {cartItems.length > 0 && (
                            <button 
...
}> = ({ items, total, onCheckout, isDineIn, isSessionActive, onUpdateQuantity, onRemoveItem }) => (
    <div className="p-5 space-y-4 pb-48">
        ...
        <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-6 bg-gray-900/95 backdrop-blur-lg border-t border-gray-800 shadow-2xl z-30">
             <div className="flex justify-between font-black text-xl mb-5">
                 <span className="text-gray-400 uppercase text-xs tracking-widest self-center">Subtotal ahora</span>
             ...
             <button disabled={items.length === 0} onClick={onCheckout} className="w-full bg-emerald-500 py-4.5 rounded-2xl font-black text-white shadow-xl shadow-emerald-500/10 active:scale-[0.98] disabled:opacity-30 uppercase tracking-widest">
                {isDineIn ? 'CONTINUAR A MESA' : 'IR AL PAGO'}
             </button>
        </div>
    </div>
);
    ...
    return (
        <form onSubmit={e => { e.preventDefault(); onOrder({ name, phone, address: { calle: '', numero: '', colonia: '' } }, 'Efectivo', proof); }} className="p-5 space-y-6 animate-fade-in pb-44">
            <div className="space-y-4 p-6 bg-gray-800/30 border border-gray-800 rounded-3xl">
                ...
                {!isSimpleOrder && <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} className="w-full bg-gray-800 border-gray-700 rounded-2xl p-4 outline-none focus:ring-1 focus:ring-emerald-500" placeholder="WhatsApp (58414...)" required />}
            </div>

            {!isSimpleOrder && (
                <div className="space-y-4 p-6 bg-gray-800/30 border border-gray-800 rounded-3xl">
                    <h3 className="text-xs font-black text-emerald-500 uppercase tracking-widest">Carga de Comprobante</h3>
                    <p className="text-[10px] text-gray-500 leading-relaxed uppercase">Si realizaste Pago Móvil o Transferencia, adjunta la captura para cerrar tu cuenta.</p>
                    <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-700 rounded-3xl cursor-pointer hover:bg-gray-800/50 transition-all">
                        {proof ? (
                            ...
                            <div className="flex flex-col items-center text-gray-500">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest">Toca para subir</span>
                            </div>
                        )}
            ...
            <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-6 bg-gray-900/95 backdrop-blur-lg border-t border-gray-800 shadow-2xl z-30">
                 <div className="flex justify-between font-black text-xl mb-5">
                    <span className="text-gray-400 uppercase text-xs tracking-widest self-center">{isFinalPayment ? 'TOTAL DE LA CUENTA' : 'A PAGAR AHORA'}</span>
                    <span>${total.toFixed(2)}</span>
                 </div>
                 <button type="submit" className={`w-full py-4.5 rounded-2xl font-black text-white flex items-center justify-center gap-3 shadow-xl transition-all active:scale-[0.98] ${isSimpleOrder ? 'bg-emerald-600 shadow-emerald-500/10' : 'bg-green-600 shadow-green-500/10'}`}>
                    {isSimpleOrder ? 'CONFIRMAR Y ENVIAR A COCINA' : <><IconWhatsapp className="h-6 w-6" /> REPORTAR PAGO FINAL</>}
                </button>
            </div>
        </form>
    );
